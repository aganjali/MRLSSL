...
namespace Simulator
{
    ...
    public static class ColladaLoader
    {
        public static Model Load(string file, Device device)
        {
            ...
            var positionsNode = document.SelectSingleNode("/c:COLLADA/c:library_geometries/c:geometry[@id='Cylinder01-lib']/c:mesh/c:source[@id='Cylinder01-POSITION']/c:float_array[@id='Cylinder01-POSITION-array']", ns);
            var indicesNode = document.SelectSingleNode("/c:COLLADA/c:library_geometries/c:geometry[@id='Cylinder01-lib']/c:mesh/c:triangles/c:p", ns);
