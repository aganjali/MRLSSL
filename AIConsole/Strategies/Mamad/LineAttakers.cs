...
{
    class LineAttakers : StrategyBase
    {
        ...
        {
            StrategyName = "M_LineAttakers_Corner";
            AttendanceSize = 4;
            About = "3 robot in Line in front of dangerZone & Simple pass shoot !!!";
        }
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model)
        {
            ...
            if (CurrentState == (int)State.First)
            {
                ...
                ShooterFirstPos = new Position2D(-3, -sgn * 0.1);

                PoserPos1 = new Position2D(-2.7, -sgn * 0.1);
                PoserPos2 = new Position2D(-2.4, -sgn * 0.1);
            ...
            else if (CurrentState == (int)State.Go)
            {
                ...
                if ((Model.BallState.Location.X < 0 && Model.BallState.Location.Y < 0) || (Model.BallState.Location.X > 0 && Model.BallState.Location.Y > 0))
                {
                    ...
                    PoserPos1 = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(vec2.AngleInRadians + (55 * Math.PI / 180), vec2.Size));
                    PoserPos2 = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(vec2.AngleInRadians + (-45 * Math.PI / 180), vec2.Size));
