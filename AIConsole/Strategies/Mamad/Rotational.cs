...
namespace MRL.SSL.AIConsole.Strategies.new_RC2017
{
    class @new : StrategyBase
    {
        ...
        {
            StrategyName = "M_Rotational_Corner";
            AttendanceSize = 4;
            About = "Rotate Around me!!!!";
        }
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model)
        {
            ...
            if (CurrentState == (int)State.First)
            {
                Vector2D v = (Position2D.Zero - GameParameters.OppGoalCenter).GetNormalizeToCopy(2);
                PoserPos1 = ShooterFirstPos.Extend(0, 0.3);
                PoserPos2 = ShooterFirstPos.Extend(0, -0.3);

                if ((Model.BallState.Location.X < 0 && Model.BallState.Location.Y < 0) || (Model.BallState.Location.X > 0 && Model.BallState.Location.Y > 0))
                {
                    ShooterFirstPos = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(v.AngleInRadians + (-50 * Math.PI / 180), v.Size));
                }
            ...
            else if (CurrentState == (int)State.Go)
            {
                ...
                vec1 = (center - GameParameters.OppGoalCenter).GetNormalizeToCopy(1.7);
                ShooterPos = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(vec1.AngleInRadians + (60 * Math.PI / 180), vec1.Size));
        ...
        public override Dictionary<int, RoleBase> RunStrategy(GameStrategyEngine engine, WorldModel Model, out Dictionary<int, CommonDelegate> Functions)
        {
            ...
            if (PoserPos1.DistanceFrom(Model.OurRobots[PoserID1].Location) < 0.05)
            {
                PoserPos1 = ShooterFirstPos + Vector2D.FromAngleSize((PoserPos1 - ShooterFirstPos).AngleInRadians + (10 * Math.PI / 180), (PoserPos1 - ShooterFirstPos).Size);
            }
            if (PoserPos2.DistanceFrom(Model.OurRobots[PoserID2].Location) < 0.05)
            {
                PoserPos2 = ShooterFirstPos + Vector2D.FromAngleSize((PoserPos2 - ShooterFirstPos).AngleInRadians + (10 * Math.PI / 180), (PoserPos2 - ShooterFirstPos).Size);
            }
            ...
            if (CurrentState == (int)State.First)
            {
                Planner.Add(ShooterID, ShooterFirstPos, ShooterAngle, PathType.UnSafe, false, true, true, true, false);
            ...
            if (CurrentState == (int)State.Go)
            {
                ...
                if (PoserPos1.DistanceFrom(Model.OurRobots[PoserID1].Location) < 0.05)
                {
                    PoserPos1 = ShooterFirstPos + Vector2D.FromAngleSize((PoserPos1 - ShooterFirstPos).AngleInRadians + (10 * Math.PI / 180), (PoserPos2 - ShooterFirstPos).Size);
                }
                if (PoserPos2.DistanceFrom(Model.OurRobots[PoserID2].Location) < 0.05)
                {
                    PoserPos2 = ShooterFirstPos + Vector2D.FromAngleSize((PoserPos2 - ShooterFirstPos).AngleInRadians + (10 * Math.PI / 180), (PoserPos2 - ShooterFirstPos).Size);
                }
                Planner.Add(PoserID1, PoserPos1, 180, PathType.Safe, true, true, true, true);
                Planner.Add(PoserID2, PoserPos2, 180, PathType.Safe, true, true, true, true);



                //  if (chip)
                //     sync.SyncChipCatch(engine, Model, PasserID, Passerpos, ShooterID, PassTarget, ShootTarget, PassSpeed, PassSpeed, RotateDelay, true, kickPowerType.Speed, false, 0);
                //  else
                //    sync.SyncDirectCatch(engine, Model, PasserID, Passerpos, ShooterID, PassTarget, ShootTarget, PassSpeed, PassSpeed, true, kickPowerType.Speed, RotateDelay, 0);
                // if (sync.InPassState)
                //    inPassState = true;

                //if (chip)
                //    sync.SyncChipCatch(engine, Model, PasserID, Passerpos, ShooterID, PassTarget, ShootTarget, PassSpeed, PassSpeed, RotateDelay, true, kickPowerType.Speed, false);
                //  else
                //    sync.SyncDirectCatch(engine, Model, PasserID, Passerpos, ShooterID, PassTarget, ShootTarget, PassSpeed, PassSpeed, true, kickPowerType.Speed, RotateDelay);
