...
namespace MRL.SSL.AIConsole.Strategies.new_RC2017
{
    class new1_2 : StrategyBase
    {
        ...
        {
            StrategyName = "M_new1_2_Middle";
            AttendanceSize = 5;
            About = " middle TO middle && middle TO corner";
        }
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model)
        {
            ...
            if (CurrentState == (int)State.First)
            {
                if (firstInState)
                {
                    ...
                    {
                        PassTarget = new Position2D(Model.BallState.Location.X, -sgn * 2.2);
                        ...
                        SecondPassTarget = new Position2D(sgn * 3.7, Model.BallState.Location.Y);
                    }
                    ...
                    {
                        PassTarget = new Position2D(Model.BallState.Location.X, -sgn * 2.3);
                        Pos4 = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(vec1.AngleInRadians + (-85 * Math.PI / 180), vec1.Size));
                        Pos5 = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(vec1.AngleInRadians + (-75 * Math.PI / 180), vec1.Size));
                        Pos6 = GameParameters.OppGoalCenter + (Vector2D.FromAngleSize(vec2.AngleInRadians + (-15 * Math.PI / 180), vec2.Size));
            ...
            {
                PassSpeed = PassSpeed = engine.GameInfo.CalculateKickSpeed(Model, PasserID, Model.BallState.Location, PassTarget, false, true);//Model.BallState.Location.DistanceFrom(PassTarget) * 0.8;
                if (firstInState)
                {
                ...
                if (!passed)
                {
                    ...
                    PassSpeed =  engine.GameInfo.CalculateKickSpeed(Model, PasserID, Model.BallState.Location, PassTarget, false, true); //Model.BallState.Location.DistanceFrom(PassTarget) * 0.8;
        ...
        public override Dictionary<int, RoleBase> RunStrategy(GameStrategyEngine engine, WorldModel Model, out Dictionary<int, CommonDelegate> Functions)
        {
            ...
            else if (CurrentState == (int)State.FirstPass)
            {
                ...
                if (flag)
                {
                    ...
                    //if (StaticRoleAssigner.AssignRole(engine, Model, PreviouslyAssignedRoles, CurrentlyAssignedRoles, Poser2ID, typeof(DefenderCornerRole3)))
                    //    Functions[Poser2ID] = (eng, wmd) => GetRole<DefenderCornerRole3>(Poser2ID).Run(eng, wmd, Poser2ID, DefenderPos1, DefenderAng1);
                   // Planner.Add(Poser2ID, SecondPassTarget, (ShootTarget - ShooterPos).AngleInDegrees, PathType.UnSafe, true, true, true, true);               
                    if (isChip)
                        sync.SyncChipCatch(engine, Model, PasserID, PasserPos, Poser1ID, PassTarget, SecondPassTarget, 3, 3, RotateDelay, true, kickPowerType.Speed, backSensor);
                    else
                        sync.SyncDirectCatch(engine, Model, PasserID, PasserPos, Poser1ID, PassTarget, SecondPassTarget, PassSpeed, PassSpeed, true, kickPowerType.Speed, RotateDelay);
                }
                else
                {
                ...
                if (passed)
                {
                    Planner.Add(PasserID, SecondPassTarget, (ShootTarget - Pos1).AngleInDegrees, PathType.Safe, true, true, true, true);
            ...
            else if (CurrentState == (int)State.SecondPass)
            {
                ...
                if (passed)
                {
                    ...
                    Planner.Add(PasserID, Pos2, (ShootTarget - Pos2).AngleInDegrees, PathType.Safe, true, true, true, true);
