...
{
    public class Attack5Strategy : StrategyBase
    {
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model)
        {
            ...
            else if (CurrentState == (int)State.Go)
            {
                if (!firstPosCalculated)
                {
                    ...
                    {
                        buPositionersPos[id1] = new Position2D(-2.6, Math.Sign(-Model.BallState.Location.Y) * 1.1);
                    ...
                    else if (shooterID == positionersID[1] || shooterID == positionersID[2])
                    {
                        ...
                        buPositionersPos[id2] = new Position2D(-2.4, Math.Sign(-Model.BallState.Location.Y) * 1.1);
                ...
                if (!passed)
                {
                    ...
                    if (chip || passTarget.X < -1.4)
                        PassSpeed = Math.Max(0.9, Model.BallState.Location.DistanceFrom(passTarget) * 0.5);
        ...
        Position2D GetPassPoint(GameStrategyEngine engine, WorldModel Model)
        {
            ...
            Position2D topLeft = new Position2D(buPositionersPos[id0].X , (Math.Sign(Model.BallState.Location.Y) >= 0 ? 1.1 : -(1.1 + heigth)));
            Position2D bestPoint = Position2D.Zero;
            chip = chipOrigin;
            engine.GameInfo.BestPassPoint(Model, shootTarget, topLeft, width, heigth, (int)(width / step), (int)(heigth / step), ref chip, ref bestPoint);
            chipOrigin = chip;
            ...
            if (shooterID == positionersID[1] || shooterID == positionersID[2])
            {
                ...
                {
                    AngleT = 80;
                    pTarget = new Position2D(Model.BallState.Location.X, Math.Sign(Model.BallState.Location.Y) * 0.1);
                }
                else
                {
                    AngleT = 60;
                    pTarget = new Position2D(-2.65, Math.Sign(Model.BallState.Location.Y) * 0.3);
                }
                //pTarget = new Position2D(-2.40, -Math.Sign(Model.BallState.Location.Y) * 0.4);//buPositionersPos[shooterID];
                chipOrigin = true;
                chip = chipOrigin;
            }
            else
        ...
        public override Dictionary<int, RoleBase> RunStrategy(GameStrategyEngine engine, WorldModel Model, out Dictionary<int, CommonDelegate> Functions)
        {
            ...
            else if (CurrentState == (int)State.FakeMove)
            {
                ...
                if (Planner.AddRotate(Model, passerID, (open2kick[id0] == 1) ? passTarget : shootTarget, passerPos, kickPowerType.Speed, PassSpeed, chip, rotateCounter, backSensor).IsInRotateDelay)
                {
                    rotateCounter++;
                }
                foreach (var item in positionersID)
            ...
            else if (CurrentState == (int)State.Go)
            {
                if (shooterID == positionersID[1] || shooterID == positionersID[2])
                {
                    ...
                    if (passed)
                    {
                        ...
                        //if ((Model.BallState.Speed.Size < 1.2 || Model.BallState.Location.DistanceFrom(Model.OurRobots[shooterID].Location) < 0.5)&& GameParameters.IsInDangerousZone(Model.BallState.Location, true, 0.1, out dist, out DistFromBorder))
                        //    ballMoved = true;

                        //if (ballMoved)
                        //{
                        //    //getBallSkill.OutGoingBackTrack(engine, Model, shooterID, shootTarget);
                        //    getBallSkill.OutGoingSideTrack(Model, shooterID, shootTarget);
