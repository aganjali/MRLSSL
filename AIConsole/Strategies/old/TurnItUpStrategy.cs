...
{
    public class TurnItUpStrategy:StrategyBase
    {
        const double tresh = 0.05, stuckTresh = 0.23, angleTresh = 2, waitTresh = 70, finishTresh = 100, initDist = 0.22, maxWaitTresh = 360, passSpeedTresh = StaticVariables.PassSpeedTresh, behindBallTresh = 0.07, fieldMargin = 0.12, faildFarPassSpeedTresh = 0.3, faildNearPassSpeedTresh = -0.05, faildBallDistSecondPass = 0.5, faildMaxCounter = 15, faildBallMovedDist = 0.06, maxFaildMovedDist = 0.2;
        ...
        Position2D PasserPos, ShooterPos, DefenderPos, GoaliePos, ShootTarget, PassTarget, firstBallPos;
        Position2D[] PositionersPos;
        double PasserAng, ShooterAng, DefenderAng, GoalieAng, PassSpeed, KickSpeed;
        double[] PositionersAng;
        Syncronizer sync;
        int Mode, faildCounter;
        bool backSensor;
        ...
        {
            StrategyName = "TurnItUp";
            AttendanceSize = 6;
            About = "this strategy will turn an attacker up from behind of 3 others!";
        }
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model)
        {
            ...
            {
                timeLimitCounter++;
                if (Model.OurRobots[PasserID].Location.DistanceFrom(PasserPos) < 0.23 
            ...
            if (CurrentState == (int)State.InitialState)
            {
                ...
                {
                    double x = -1, y = 1;
                    PositionersPos[0] = new Position2D(x, sgn * y);
                    PositionersPos[1] = new Position2D(x, 0);
                    PositionersPos[2] = new Position2D(x, -sgn * y);
                    ...
                    ShooterPos = new Position2D(1.2, 0);
            ...
            else if (CurrentState == (int)State.Go)
            {
                ...
                {
                    if (Mode == 0)
                        DefenderID = PositionersID[2];
                    else
                    {
                        DefenderID = ShooterID;
                        ShooterID = PositionersID[2];
                    }
                    double width = 0.8, heigth = 0.8, step = 0.2;
                    Position2D topLeft = new Position2D(0, (Model.BallState.Location.Y > 0)?0.5:-(0.5 + heigth));
                    ...
                    double margin = 0.25;
                    Position2D p0 = new Position2D(-2.1, 0.9 * sgn);
                    double d = GameParameters.SafeRadi(new SingleObjectState(-p0, Vector2D.Zero, 0), margin);
                    PositionersPos[0] = GameParameters.OppGoalCenter + (p0 - GameParameters.OppGoalCenter).GetNormalizeToCopy(d);
                    p0 = new Position2D(-2.1, -sgn * 0.9);
                    ...
                    PositionersPos[1] = GameParameters.OppGoalCenter + (p1 - GameParameters.OppGoalCenter).GetNormalizeToCopy(d);

                    //PositionersPos[0] = new Position2D(-2.2, sgn * 0.8);
                    //PositionersPos[1] = new Position2D(-2.1, -sgn * 0.7);
                    //PositionersPos[2] = new Position2D(-2.25, -sgn * 0.9);
                    PositionersAng[0] = 180;// (ShootTarget - PositionersPos[0]).AngleInDegrees;
                    PositionersAng[1] = 180;//(ShootTarget - PositionersPos[1]).AngleInDegrees;
                    PositionersAng[2] = (ShootTarget - PositionersPos[2]).AngleInDegrees;
                ...
                if (!passed)
                {
                    ...
                    PassSpeed = 4;// engine.GameInfo.CalculateKickSpeed(Model, PasserID, Model.BallState.Location, PassTarget, isChip, true);
                    if (isChip)
                        PassSpeed = /*Model.BallState.Location.DistanceFrom(PassTarget) * 0.55*/ 1.25;
                }
                else
                {
                    double r = GameParameters.SafeRadi(new SingleObjectState(-PositionersPos[0], Vector2D.Zero, 0), 0.0);
        ...
        public override Dictionary<int, RoleBase> RunStrategy(GameStrategyEngine engine, WorldModel Model, out Dictionary<int, CommonDelegate> Functions)
        {
            ...
            if (CurrentState == (int)State.InitialState)
            {
                ...
                if (StaticRoleAssigner.AssignRole(engine, Model, PreviouslyAssignedRoles, CurrentlyAssignedRoles, Model.GoalieID, typeof(GoalieCornerRole)))
                    Functions[Model.GoalieID.Value] = (eng, wmd) => GetRole<GoalieCornerRole>(Model.GoalieID.Value).Run(engine, Model, Model.GoalieID.Value, GoaliePos, GoalieAng, new DefenceInfo(), DefenderPos, DefenderID, true);
            }
            else if (CurrentState == (int)State.Go)
            {
                if (Mode == 0)
                {
                    RotateDelay = 20;
                    if (Model.OurRobots[PositionersID[2]].Location.DistanceFrom(PositionersPos[2]) < 0.23)
                        goBack = true;
                    if (goBack)
                    {
                        if (isChip)
                    ...
                    if (goBack)
                    {
                        if (Model.OurRobots[DefenderID].Location.DistanceFrom(DefenderPos) > 1)
                        {
                            Planner.ChangeDefaulteParams(DefenderID, false);
                            Planner.SetParameter(DefenderID, 7, 6);
                            Planner.Add(DefenderID, DefenderPos, DefenderAng, PathType.UnSafe, false, true, true, false);
                        }
                        else
                        {
