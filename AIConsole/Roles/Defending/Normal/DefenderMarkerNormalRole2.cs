...
{
    public class DefenderMarkerNormalRole2 : RoleBase, IMarkerDefender
    {

        bool first = true, calculateCost = false;

        MarkerState current = MarkerState.CoverOurGoal;
        ...
        public void Mark(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D target, double teta)
        {
            ...
            FreekickDefence.PreviousPositions[typeof(DefenderMarkerNormalRole2)] = pos;
        ...
        public override void DetermineNextState(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            if (FreekickDefence.CurrentInfos.Any(a => a.RoleType == this.GetType()))
            {
                ...
                {
                    if (OurFreeKickMarkerUtils.PassedToOpponent(Model, RobotID, FreekickDefence.LastOppToMark2))
                    {
                ...
                {
                    if (!OurFreeKickMarkerUtils.PassedToOpponent(Model, RobotID, FreekickDefence.LastOppToMark2) || (FreekickDefence.LastOppToMark2.HasValue && lastopploc.DistanceFrom(Model.Opponents[FreekickDefence.LastOppToMark2.Value].Location) > 0.3))
                    {
        ...
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            ...
            new NewSupporterRole(), new DefenderMarkerNormalRole2()};
