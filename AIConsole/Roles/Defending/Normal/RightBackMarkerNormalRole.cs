...
{
    class RightBackMarkerNormalRole : RoleBase
    {
#if NEW
        ...
        public void Mark(GameStrategyEngine engine, WorldModel Model, int RobotID, int? MarkID)
        {
            ...
            if (CurrentState == (int)MarkStateInNear.Attacker)
            {
                ...
                {
                    double moveAngle = (Model.BallState.Location - AtackerInfo.PosToGo2.Value).AngleInDegrees;
                    Planner.Add(RobotID, AtackerInfo.PosToGo2.Value, moveAngle, PathType.UnSafe, true, true, true, true);
                }
                else
                {
            ...
            else
            {
                ...
                if (CurrentState == (int)MarkStateInNear.Disrupt)//oh oh oh .....boom
                {
                    Planner.Add(RobotID, target, moveAngle, PathType.UnSafe, false, false, dangerZone, false);
                }
                DataBridge.RBMarkerRoleID = RobotID;
        ...
        public Position2D CostFunction(GameStrategyEngine engine, WorldModel Model, int RobotID, out double retAngle, out bool cutBall, bool fartarget)
        {
            ...
            {
                markID = FreekickDefence.OppToMark2.Value;
                markSpeed = Model.Opponents[FreekickDefence.OppToMark2.Value].Speed;
                markPos = Model.Opponents[FreekickDefence.OppToMark2.Value].Location;
            ...
            if (markID != 1000)
            {
                ...
                if (CurrentState == (int)MarkStateInNear.Cut)
                {
                    ...
                    {
                        lastTarget = LastCutpos + (LastCutpos - GameParameters.OppGoalRight).GetNormalizeToCopy(0.09);
                    }
                    else
                    ...
                    //DrawingObjects.AddObject(new Circle(cutpos, 0.02f, new Pen(Brushes.BlueViolet, 0.02f)), "684613");
                    ...
                    if (Model.OurRobots[RobotID].Location.DistanceFrom(Model.BallState.Location) < .3 && lastTarget != Position2D.Zero)
                    {
                        if (Model.BallState.Location.DistanceFrom(lastTarget) > Model.BallState.Location.DistanceFrom(Model.Opponents[markID].Location))
                        {
                            ...
                            //DrawingObjects.AddObject(new Line(Model.Opponents[markID].Location, Model.Opponents[markID].Location + (Model.Opponents[markID].Location - Model.BallState.Location)), "dskjghfksuiah54f56ds");                          
                        }
                        else
                            GetSkill<GetBallSkill>().Perform(engine, Model, RobotID, Position2D.Zero);
                        Planner.AddKick(RobotID, kickPowerType.Power, kickPower, true, false);

                        #region
                        //GetSkill<GetBallSkill>().SetAvoidDangerZone(false, true);
                        //Position2D tar = GameParameters.OppGoalCenter;
                        ...
                        //else
                        //{
                        //    tar = GameParameters.OppGoalCenter;
                        //}
                        //GetSkill<GetBallSkill>().OutGoingSideTrack(Model, RobotID, tar);

                        //Obstacles obstacle = new Obstacles(Model);
                        //obstacle.AddObstacle(1, 0, 0, 0, new List<int>() { RobotID }, null);
                        //Vector2D v = Vector2D.FromAngleSize(Model.OurRobots[RobotID].Angle.Value * Math.PI / 180, 0.35);
                        //kickPower = 150;

                        //if (Model.BallState.Location.X > GameParameters.OurGoalCenter.X - 0.1 || Math.Abs(Model.OurRobots[RobotID].Angle.Value) < 100 || obstacle.Meet(Model.BallState, new SingleObjectState(Model.BallState.Location + v, Vector2D.Zero, 0), 0.022))
                        //    kickPower = 0;
                        //Planner.AddKick(RobotID, kickPowerType.Power, kickPower, (kickPower > 0) ? true : false, false);
                        #endregion

                    }
                    DrawingObjects.AddObject(new Circle(lastTarget, 0.09, new Pen(Brushes.Blue, 0.02f)), "df53g4df5s64g56df46g5");
                ...
                {
                    if (FreekickDefence.OppInRBArea && FreekickDefence.OppInRBAreaFake && markIDFake != 1000 && Model.Opponents.ContainsKey(markIDFake))
                    {
                        ...
                        retAngle = (GameParameters.OppGoalCenter - Model.OurRobots[RobotID].Location).AngleInDegrees;
                        lastTarget = p;
                        DrawingObjects.AddObject(new Circle(p, 0.09, new Pen(Brushes.Red, 0.02f)), "fgdfs4556g46g6g");
                    }
                    ...
                    //{
                    //    retAngle = (Model.Opponents[markID].Location - Model.OurRobots[RobotID].Location).AngleInDegrees;
                    //    lastTarget = GameParameters.OurGoalRight + (markPos - GameParameters.OurGoalCenter).GetNormalizeToCopy(GameParameters.SafeRadi(new SingleObjectState(markPos, markSpeed, 0.0f), 2));
                    //    lastTarget = new Position2D(lastTarget.X, Model.Opponents[markID].Location.Y);
                    //}
                }
                #endregion
                ...
                if (CurrentState == (int)MarkStateInNear.NewPositioning)
                {
                    ...
                    //    lastTarget = GameParameters.OurGoalCenter + v.GetNormalizeToCopy(1.5);
                    //}
                    //else
                    //{
                ...
                #region MarkInRegion
                //if (CurrentState==(int)MarkStateInNear.MarkInRegion)
                //{
                //    Vector2D vectFromMarkIdToMarkRegion = Model.Opponents[FreekickDefence.OppToMarkRegion.Value].Location - Model.Opponents[markID].Location;
                //    Vector2D vectFromMarkIdToGoalRight = GameParameters.OurGoalRight - Model.Opponents[markID].Location;
                //    Line bisectorLine = Vector2D.Bisector(vectFromMarkIdToGoalRight, vectFromMarkIdToMarkRegion, Model.Opponents[markID].Location);
                //    Vector2D bisectorVect = bisectorLine.Tail - bisectorLine.Head;
                //    Position2D p = Model.Opponents[markID].Location + bisectorVect.GetNormalizeToCopy(2);
                //    //DrawingObjects.AddObject(p, p.X.ToString() + "5476");
                //    lastTarget = p;
                //}
                #endregion
            }
            ...
            {
                #region NewPositioninig
                DrawingObjects.AddObject(new StringDraw("markID is Null", new Position2D(-0.2, 3.5)), "sdfg54d5s36fg");
                retAngle = (GameParameters.OppGoalCenter - Model.OurRobots[RobotID].Location).AngleInDegrees;
                lastTarget = new Position2D(2, -1.5);
                #endregion
            }
            //DrawingObjects.AddObject(new StringDraw("lastTarget real: " + lastTarget.toString(), Position2D.Zero.Extend(0.6, 0)), "ds3f56g4sa586tg");
            ...
            //DrawingObjects.AddObject(new StringDraw("lastTarget CheckForStopZone: " + lastTarget.toString(), Position2D.Zero.Extend(0.7, 0)), "d3xe5ryh4u");
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            #region defence
            {
                if (FreekickDefence.rightFirstSet)
                {
                    if (FreekickDefence.OppToMark2.HasValue)
                    {
                        FreekickDefence.Area a = new FreekickDefence.Area();
                        FreekickDefence.OppToMark2 = a.OpponnentToMarkeRobots(engine, Model, false, true);
                        if (FreekickDefence.OppToMark2.HasValue)
                        {
                            markID = FreekickDefence.OppToMark2.Value;
                            markSpeed = Model.Opponents[FreekickDefence.OppToMark2.Value].Speed;
                    ...
                    if (FreekickDefence.OppToMark2.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
                    {
                        markID = FreekickDefence.OppToMark2.Value;
                    }
                    if (FreekickDefence.OppToMark2Fake.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2Fake.Value))
                    {
                        markIDFake = FreekickDefence.OppToMark2Fake.Value;
                    }
                }
                if (FreekickDefence.OppToMark2.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
                {
                    FreekickDefence.rightFirstSet = false;
                }
                else
                {
                    FreekickDefence.rightFirstSet = true;
                    FreekickDefence.OppToMark2 = null;
                    FreekickDefence.OppToMark2Fake = null;
                    markID = 1000;
                    markIDFake = 1000;
                }
                ...
                foreach (var item in Model.Opponents.Keys)
                {
                    if (Model.Opponents[item].Location.X > 0)
                    {
                        ...
                        intersectP = oppLineX.IntersectWithLine(FreekickDefence.RBLine);
                        if (intersectP.HasValue)
                        {
                            if (Model.Opponents[item].Location.Y < intersectP.Value.Y && !(engine.GameInfo.OppTeam.BallOwner.HasValue && engine.GameInfo.OppTeam.BallOwner.Value == item)
                                && ((FreekickDefence.OppToMark1.HasValue && item != FreekickDefence.OppToMark1) || !FreekickDefence.OppToMark1.HasValue))
                            {
                                temp.Add(item);
                            }
                        }
                ...
                FreekickDefence.oppCountRight = temp.Count;
                if (FreekickDefence.oppCountRight > 1)
                {
                    FreekickDefence.rightFirstSet = true;
                }
                    ...
                    if (Model.Opponents.ContainsKey(markID) && Model.Opponents[markID].Location.X > -0.2)
                    {
                        ...
                        if (intersectP.HasValue)
                        {
                            if (Model.Opponents[markID].Location.Y < intersectP.Value.Y)
                                FreekickDefence.OppInRBArea = true;
                            else
                                FreekickDefence.OppInRBArea = false;
                        }
                    }
                    else
                    {
                    ...
                    if (Model.Opponents.ContainsKey(markIDFake) && Model.Opponents[markIDFake].Location.X > -0.2)
                    {
                        ...
                        if (intersectP.HasValue)
                        {
                            if (Model.Opponents[markIDFake].Location.Y < intersectP.Value.Y && !(engine.GameInfo.OppTeam.BallOwner.HasValue && engine.GameInfo.OppTeam.BallOwner.Value == markIDFake)
                                && ((FreekickDefence.OppToMark1.HasValue && markIDFake != FreekickDefence.OppToMark1) || !FreekickDefence.OppToMark1.HasValue))
                                FreekickDefence.OppInRBAreaFake = true;
                            else
                                FreekickDefence.OppInRBAreaFake = false;
                        }
                    }
                    else
                    {
                ...
                foreach (var item in Model.Opponents.Keys)
                {
                    if (Model.Opponents.ContainsKey(item) && Model.Opponents[item].Location.X > -0.2 && (FreekickDefence.OppToMark1.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark1.Value) && item != FreekickDefence.OppToMark1.Value))
                    {
                        oppLineX = new Line(new Position2D(Model.Opponents[item].Location.X, GameParameters.OurLeftCorner.Y), new Position2D(Model.Opponents[item].Location.X, GameParameters.OurGoalLeft.Y));
                        intersectP = oppLineX.IntersectWithLine(FreekickDefence.RBLine);
                        if (intersectP.HasValue)
                        {
                ...
                {
                    if (CurrentState != (int)MarkStateInNear.NearFront && CurrentState != (int)MarkStateInNear.NearBehind)
                    {
                        FreekickDefence.rightFirstSet = true;
                    }
                    OppInMyDefenceArea = true;
                }
                FreekickDefence.OppInRBAreaReal = FreekickDefence.OppInRBArea;
                if (!FreekickDefence.OppInRBArea /*&& FreekickDefence.OppInLBAreaReal*/ && markID != 1000 && Model.Opponents.ContainsKey(markID) && Model.Opponents[markID].Location.X > -0.2 && !OppInMyDefenceArea)
                {
                    FreekickDefence.OppInRBArea = true;
                ...
                if (FreekickDefence.OppToMark2.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
                {
                    ...
                    #region defece
                    if ((FreekickDefence.OppInRBArea && markID != 1000 && !FreekickDefence.OppInRBAreaFake) || markID == markIDFake)
                    {
                        #region 1
                        markID = FreekickDefence.OppToMark2.Value;
                        markSpeed = Model.Opponents[FreekickDefence.OppToMark2.Value].Speed;
                        ...
                        if (intersect.HasValue)
                        {
                            ...
                            if (GameParameters.IsInDangerousZone(intersect.Value, false, FreekickDefence.AdditionalSafeRadi, out x, out y))
                                intersect = GameParameters.OurGoalCenter + (markPos - GameParameters.OurGoalCenter).GetNormalizeToCopy(GameParameters.SafeRadi(new SingleObjectState(markPos, markSpeed, 0.0f), 0.35));
                            intersectG = intersect.Value;
                            int counter = 0;
                            bool ourconflict = true;
                            bool oppconflict = true;
                        ...
                        if (markPos != Position2D.Zero)
                        {
                            if (!disrupt)
                            {
                                ...
                                //DrawingObjects.AddObject(ballspeed, "sdfkjsahdfou43423");
                                //DrawingObjects.AddObject(new Circle(Model.BallState.Location, 0.02,new Pen(Brushes.Red,0.02f)), "sjadsfdadaf53456");
                                //DrawingObjects.AddObject(new Circle(Model.BallState.Location + Model.BallState.Speed.GetNormalizeToCopy(3), 0.02, new Pen(Brushes.Red, 0.02f)), "dsgdsagsdg45s64");
                                double dist2 = ballspeed.Distance(Model.OurRobots[RobotID].Location);
                                if (CurrentState == (int)MarkStateInNear.Cut)
                                {
                                    //DrawingObjects.AddObject(new StringDraw("InnerProduct: " + (Model.BallState.Speed.GetNormnalizedCopy().InnerProduct((Model.OurRobots[RobotID].Location - Model.BallState.Location).GetNormnalizedCopy()) < 0).ToString(), GameParameters.OurRightCorner.Extend(0.1, 0)), "xdfh5dft465j");
                                    //DrawingObjects.AddObject(new StringDraw("ballspeed size lower than 0.5: " + (Model.BallState.Speed.Size < 0.50).ToString(), GameParameters.OurRightCorner.Extend(0.2, 0)), "fghjdfg3h2jdy4gfj");
                                    if ((Model.BallState.Speed.Size < .7 && Model.BallState.Speed.GetNormnalizedCopy().InnerProduct((Model.OurRobots[RobotID].Location - Model.BallState.Location).GetNormnalizedCopy()) < 0) || Model.BallState.Speed.Size < 0.5)
                                    {
                                        CurrentState = (int)MarkStateInNear.NearFront;
                                    }
                                }
                                //else if (CurrentState == (int)MarkStateInNear.MarkInRegion)
                                //{
                                //    if (engine.GameInfo.OppTeam.BallOwner.HasValue && Model.Opponents.ContainsKey(engine.GameInfo.OppTeam.BallOwner.Value))
                                //    {
                                //        if (!(FreekickDefence.OppToMarkRegion.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMarkRegion.Value)))
                                //        {
                                //            CurrentState = (int)MarkStateInNear.FarFront;
                                //        }
                                //        else if (FreekickDefence.OppToMarkRegion.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMarkRegion.Value)
                                //                 && Model.Opponents[FreekickDefence.OppToMarkRegion.Value].Location.Y > 0)
                                //        {
                                //            if (Model.Opponents[markID].Location.DistanceFrom(Model.Opponents[engine.GameInfo.OppTeam.BallOwner.Value].Location) > 0.5
                                //                && Model.Opponents[markID].Location.Y > Model.Opponents[engine.GameInfo.OppTeam.BallOwner.Value].Location.Y)
                                //            {
                                //                CurrentState = (int)MarkStateInNear.NearFront;
                                //            }
                                //            if (Model.Opponents[markID].Location.X > Model.Opponents[engine.GameInfo.OppTeam.BallOwner.Value].Location.X)
                                //            {                                            
                                //                CurrentState = (int)MarkStateInNear.NearFront;
                                //            }
                                //        }
                                ...
                                {
                                    //DrawingObjects.AddObject(new StringDraw("NearFront: " + (CurrentState == (int)MarkStateInNear.NearFront).ToString(), GameParameters.OurLeftCorner.Extend(0.1, 0)), "dfhkjgsdui");
                                    //DrawingObjects.AddObject(new StringDraw("meet: " + (!meet).ToString(), GameParameters.OurLeftCorner.Extend(0.2, 0)), "g54rty897reg");
                                    //DrawingObjects.AddObject(new StringDraw("dist2 < 0.30: " + (dist2 < 0.30).ToString(), GameParameters.OurLeftCorner.Extend(0.3, 0)), "rhyst4h56s46gsghdfss");
                                    //DrawingObjects.AddObject(new StringDraw("!obs.Meet(Model.BallState, Model.Opponents[markID], MotionPlannerParameters.BallRadi): " + (!obs.Meet(Model.BallState, Model.Opponents[markID], MotionPlannerParameters.BallRadi)).ToString(), GameParameters.OurLeftCorner.Extend(0.4, 0)), "styhaehbadfgh1456");
                                    //DrawingObjects.AddObject(new StringDraw("InnerProduct: " + (Model.BallState.Speed.GetNormnalizedCopy().InnerProduct((Model.Opponents[markID].Location - Model.BallState.Location).GetNormnalizedCopy()) > 0).ToString(), GameParameters.OurLeftCorner.Extend(0.5, 0)), "ftghtsreh4ts56");
                                    //DrawingObjects.AddObject(new StringDraw("dist2: " + (dist2).ToString(), GameParameters.OurLeftCorner.Extend(0.6, 0)), "dfgdf5sag4d5s6ae4");
                                    //DrawingObjects.AddObject(new StringDraw("InnerProduct value: " + (Model.BallState.Speed.GetNormnalizedCopy().InnerProduct((Model.Opponents[markID].Location - Model.BallState.Location).GetNormnalizedCopy())).ToString(), GameParameters.OurLeftCorner.Extend(0.7, 0)), "dsgh4stre8yh798y");
                                    ...
                                    //}
                                    else if (!((FarTarget - GameParameters.OurGoalCenter).Size < (markPos - GameParameters.OurGoalCenter).Size) && CurrentState == (int)MarkStateInNear.IntheWay)
                                    {
                                        CurrentState = (int)MarkStateInNear.NearFront;
                                    }
                                    ...
                                    else if (CurrentState == (int)MarkStateInNear.FarFront && new Line(FarTarget, GameParameters.OurGoalCenter).Distance(Model.OurRobots[RobotID].Location) < 0.35 && FarTarget != Position2D.Zero)
                                    {
                                        CurrentState = (int)MarkStateInNear.NearFront;
                                    }
                                    ...
                                    //if (CurrentState == (int)MarkStateInNear.NearFront || CurrentState == (int)MarkStateInNear.NearFront)
                                    //{
                                    //    if (engine.GameInfo.OppTeam.BallOwner.HasValue && Model.Opponents.ContainsKey(engine.GameInfo.OppTeam.BallOwner.Value))
                                    //    {
                                    //        if (FreekickDefence.OppToMarkRegion.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMarkRegion.Value))
                                    //        {
                                    //            if (Model.Opponents[markID].Location.DistanceFrom(Model.Opponents[engine.GameInfo.OppTeam.BallOwner.Value].Location) < 0.5
                                    //                && Model.Opponents[markID].Location.X < Model.Opponents[engine.GameInfo.OppTeam.BallOwner.Value].Location.X 
                                    //                && Model.Opponents[markID].Location.Y < Model.Opponents[engine.GameInfo.OppTeam.BallOwner.Value].Location.Y)
                                    //            {
                                    //                CurrentState = (int)MarkStateInNear.MarkInRegion;
                                    //            }
                                    //        }
                                    //    }
                                    //}
                                    ...
                                    {
                                        bool MTime1 = false;// arrivin time in the targetpos     
                                        ...
                                        {
                                            if ((CurrentState == (int)MarkStateInNear.FarBehind || CurrentState == (int)MarkStateInNear.FarFront) && FreekickDefence.OppInRBArea && ballDistFromOpp < 0.25 && MTime1 && DTime2)
                                            {
                    ...
                    else if (FreekickDefence.OppInRBArea && markID != 1000 && FreekickDefence.OppInRBAreaFake && markIDFake != 1000)
                    {
                        #region 2
            ...
            #region debug
            DrawingObjects.AddObject(new StringDraw(((MarkStateInNear)CurrentState).ToString(), new Position2D(-.1, -3.5)), "35621478538642");
            DrawingObjects.AddObject(new StringDraw("OppInRBArea: " + (FreekickDefence.OppInRBArea == true ? "True" : "False"), new Position2D(0, -3.5)), "56462145673");
            DrawingObjects.AddObject(new StringDraw("FirstSet: " + (FreekickDefence.rightFirstSet == true ? "True" : "False"), new Position2D(0.1, -3.5)), "153613643");
            DrawingObjects.AddObject(new StringDraw("markID: " + markID.ToString(), new Position2D(0.2, -3.5)), "9724987999");
            if (FreekickDefence.OppToMark2.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
            {
                DrawingObjects.AddObject(new StringDraw("oppToMark2 : " + FreekickDefence.OppToMark2.Value.ToString(), new Position2D(0.3, -3.5)), "9724987999");
            }
            if (FreekickDefence.OppToMark2Fake.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2Fake.Value))
            {
                DrawingObjects.AddObject(new StringDraw("oppToMark1Fake : " + FreekickDefence.OppToMark2Fake.Value.ToString(), new Position2D(0.4, -3.5)), "972498799fasd4eftg9");
            }
            #endregion
        }
        ...
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            ...
            {
                DrawingObjects.AddObject(new StringDraw("marker dist : " + (Model.Opponents[markID].Location.DistanceFrom(new Position2D(Model.Opponents[markID].Location.X, 0)) < 0.7).ToString(), GameParameters.OurRightCorner.Extend(0.1, 0)), "kjghafhdsakf56");
                DrawingObjects.AddObject(new StringDraw("InnerProduct : " + (Model.Opponents[FreekickDefence.OppToMark1.Value].Speed.GetNormnalizedCopy().InnerProduct((Model.Opponents[FreekickDefence.OppToMark2.Value].Speed).GetNormnalizedCopy()) < 0).ToString(), GameParameters.OurRightCorner.Extend(0.2, 0)), "dfsad54564sf");
                if (Model.Opponents[markID].Location.DistanceFrom(new Position2D(Model.Opponents[markID].Location.X, 0)) < 0.7
                    && Model.Opponents[FreekickDefence.OppToMark2.Value].Speed.GetNormnalizedCopy().InnerProduct((Model.Opponents[FreekickDefence.OppToMark1.Value].Speed).GetNormnalizedCopy()) < 0)
                {
                    FreekickDefence.SwitchRBMarkerToLBMarker = true;
                }
                else
                    FreekickDefence.SwitchRBMarkerToLBMarker = false;

            }
            if (FreekickDefence.SwitchRBMarkerToLBMarker == false)
            {
            ...
            {
                Line l = new Line(GameParameters.OurGoalCenter, Model.OurRobots[FreekickDefence.StaticRBID.Value].Location);
                Position2D? i = GameParameters.LineIntersectWithDangerZone(l, true).FirstOrDefault();
                if (i.HasValue && i.Value != Position2D.Zero)
                {
                    if (Model.OurRobots[FreekickDefence.StaticRBID.Value].Location.DistanceFrom(i.Value) < .25)
                    {
                        FreekickDefence.SwitchCBMarkerToRBMarker = true;
                    }
                    else
                    {
                        FreekickDefence.SwitchCBMarkerToRBMarker = false;
                    }
                }
                else
                {
            ...
            DrawingObjects.AddObject(new StringDraw("Switch RBMarker To LBMarker : " + (FreekickDefence.SwitchRBMarkerToLBMarker).ToString(), GameParameters.OurRightCorner.Extend(0.3, 0)), "dft3h5d4ftr5uyd");
            DrawingObjects.AddObject(new StringDraw("Switch CBMarker To LBMarker : " + (FreekickDefence.SwitchCBMarkerToRBMarker).ToString(), GameParameters.OurRightCorner.Extend(0.4, 0)), "d53fgsdv56dfg4sdfb");

            if (Model.Opponents.Count > 1)
            {
                List<RoleBase> res = new List<RoleBase>() { new RightBackMarkerNormalRole() };//, new CenterBackNormalRole() };
                if (FreekickDefence.SwitchRBMarkerToLBMarker)
                {
                    res.Add(new LeftBackMarkerNormalRole());
                }
            ...
            return new List<RoleBase>() { new RightBackMarkerNormalRole() };
            #region
            //    List<RoleBase> res = new List<RoleBase>() { new RightBackMarkerNormalRole(),
        ...
        public override double CalculateCost(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            ...
            //DrawingObjects.AddObject(new StringDraw("Cost Right:" + cost.ToString(), Model.OurRobots[RobotID].Location.Extend(-0.40, 0), true), RobotID.ToString()+"6832");
            return cost;
        }
