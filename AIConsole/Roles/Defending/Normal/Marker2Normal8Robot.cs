...
namespace MRL.SSL.AIConsole.Roles
{
    class MarkerAttackerRole2 : RoleBase
    {
        ...
        public void Perform(GameStrategyEngine engin, WorldModel Model, int RobotID, double markRegion, int? _oppMarkID, List<int> oppAttackerIds, List<int> oppValue1, List<int> oppValue2, int? field)
        {
            ...
            else
            {
                ...
                if ((oppValue1.Count == 1 && oppValue2.Count == 0) || (oppValue1.Count == 0 && oppValue2.Count == 0))
                {
                    ...
                    //ok
                    if (Model.BallState.Location.X > -0.3 &&/* Model.BallState.Location.Y > 0*/field == 1)
                    {
                        target = new Position2D(3, -2.25);
                    }
                }
                if (oppValue2.Count == 0 && oppValue1.Count >= 2)
                {
                    oppMarkID = oppValue1[1];
                }
                if (oppValue1.Count == 0 && oppValue2.Count >= 3)
                {
                    oppMarkID = oppValue2[0];
                }
                if (oppAttackerIds.Count != 0)
                {
                    if (Model.Opponents[oppMarkID.Value].Location.X < -0.3)
                    {
                        // ino dorost kon
                        if (Model.BallState.Location.X < -0.3 &&/* Model.BallState.Location.Y < 0*/field == 2)
                        {
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            if (Model.BallState.Location.X < -0.6 || !oppBallOwner && !isInDangerzone)
            {
        ...
        {
            double cost;
            cost = Model.OurRobots[RobotID].Location.DistanceFrom(NormalSharedState.CommonInfo.NormalAttackerMarker2Target);
            return cost;
        }
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            List<RoleBase> res = new List<RoleBase>() { new Marker1Normal8Robot(), new MarkerAttackerRole2(), new ActiveRole2017(), new NewSupporter2Role(), new ZjuSupporterRole() };
        ...
        public Position2D CalculateTarget(GameStrategyEngine engine, WorldModel Model, int RobotID)
        {
            ...
            else
            {
                ...
                if (CurrentState == (int)State.markFar)
                {
                    ...
                    if (flagdraw)
                    {
                        //DrawingObjectss.AddObject(leftGoalline);
                        //DrawingObjectss.AddObject(rightGoalline);
                        //DrawingObjectss.AddObject(new Circle(leftGoalpos, 0.02f));
                        //DrawingObjectss.AddObject(new Circle(rightGoalpos, 0.02f));
                        //DrawingObjectss.AddObject(new Circle(leftOpp, 0.02f));
                        //DrawingObjectss.AddObject(new Circle(rightOpp, 0.02f));
                        //DrawingObjectss.AddObject(new Line(rightOpp, leftOpp));
                        //DrawingObjectss.AddObject(new Line(leftGoalpos, leftOpp));
                        //DrawingObjectss.AddObject(new Line(rightOpp, rightGoalpos));
                        // //DrawingObjectss.AddObject(new Circle(bet, 0.02f));
                        //DrawingObjectss.AddObject(new Circle(rightIntersect.Value, 0.05f, new Pen(Brushes.Red, 0.02f)));
                        //DrawingObjectss.AddObject(new Circle(leftIntersect.Value, 0.05f, new Pen(Brushes.Blue, 0.02f)));
                        //DrawingObjectss.AddObject(new Circle(target, 0.02f));
                    }
                }
                #endregion
                #region cutball
                if (CurrentState == (int)State.cutBall)
                {
                    ...
                    if (flagdraw)
                    {
                        //DrawingObjectss.AddObject(new StringDraw("cutFlag: " + cutFlag.ToString(), new Position2D(0.1, 3)));
                        //DrawingObjectss.AddObject(new StringDraw("intersect: " + intersect.ToString(), new Position2D(0.2, 3)));
                        //DrawingObjectss.AddObject(new Circle(intersect, 0.1, new Pen(Brushes.Red, 0.02f)));
                        //DrawingObjectss.AddObject(new StringDraw("velocity: " + velocity.ToString(), new Position2D(0.3, 3)));
                        //DrawingObjectss.AddObject(new StringDraw("ballcoeff: " + ballcoeff.ToString(), new Position2D(0.4, 3)));
                        //DrawingObjectss.AddObject(new StringDraw("robotCoeff: " + robotCoeff.ToString(), new Position2D(0.5, 3)));
                        //DrawingObjectss.AddObject(new StringDraw("robotIntersectTime: " + robotIntersectTime.ToString(), new Position2D(0.6, 3)));
                    }
                }
                #endregion
                #region Stop
                ...
                if (CurrentState == (int)State.goBack)
                {
                    if (oppMarkID.HasValue && Model.Opponents.ContainsKey(oppMarkID.Value))
                    {
                        ...
                        if (Model.Opponents[oppMarkID.Value].Location.X < 1.5 && testrole)
                        {
                            ...
                            //DrawingObjectss.AddObject(l1, "as32d4s3a2d321sa432d");
                            Line l2 = new Line(new Position2D(1.5, -3), new Position2D(1.5, 3));
                            //DrawingObjectss.AddObject(l2, "asfdsaf564ds6f4awqwde");
                            Position2D? myIntersect = l1.IntersectWithLine(l2);
                            if (myIntersect.HasValue)
                            {
                                //DrawingObjectss.AddObject(new Circle(myIntersect.Value, 0.09), "sr5t3yg45rawtaetraetsre");
                ...
                if (CurrentState == (int)State.behind)
                {
                    ...
                    ////DrawingObjectss.AddObject(new Circle(target, 0.1), "shdjhd");
                }
                #endregion
                #region regional
                if (CurrentState == (int)State.regional)
            ...
            //DrawingObjectss.AddObject(new StringDraw(target.toString(), Position2D.Zero.Extend(0.1, 0)));
        ...
        private double predicttime(WorldModel Model, int RobotID, Position2D initialpos, Position2D lastpos)
        {
            ...
            if (initialstate.DistanceFrom(target) > deccelDX + daccel)
            {
                ...
                if (currentPos.DistanceFrom(initialstate) < daccel)
                {
                    //1
                    total = ta + tc2 + Td;
                    ////DrawingObjectss.AddObject(new StringDraw("1", Model.OurRobots[RobotID].Location.Extend(1.1, 0)), "2145665445496789456");
                }
                if (currentPos.DistanceFrom(target) > deccelDX && currentPos.DistanceFrom(initialstate) > daccel)
                {
                    //4
                    total = tc + Td;
                    ////DrawingObjectss.AddObject(new StringDraw("4", Model.OurRobots[RobotID].Location.Extend(1.1, 0)), "54975645696854645664564456");
                }
                if (currentPos.DistanceFrom(target) < deccelDX)
                {
                    //3
                    total = td;
                    ////DrawingObjectss.AddObject(new StringDraw("3", Model.OurRobots[RobotID].Location.Extend(1.1, 0)), "546464564645456984566");
                }
            }
            else
            {
                if (currentPos.DistanceFrom(initialstate) < daccel)
                {
                    //2
                    total = ta + Td;
                    ////DrawingObjectss.AddObject(new StringDraw("2", Model.OurRobots[RobotID].Location.Extend(1.1, 0)), "9876454652132");
                }
                else
                {
                    //3
                    total = td;
                    ////DrawingObjectss.AddObject(new StringDraw("3", Model.OurRobots[RobotID].Location.Extend(1.1, 0)), "56413121364564");
                }
            }
            lasttarget = target;
