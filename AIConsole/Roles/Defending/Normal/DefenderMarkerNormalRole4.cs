...
{
    public class DefenderMarkerNormalRole4 : RoleBase, IMarkerDefender
    {
        ...
        public void Mark(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D target, double teta)
        {
            ...
            Position2D pos = CalculateTarget(engine, Model, RobotID, inf, target, teta, out Teta, FreekickDefence.LastOppToMark4);
            FreekickDefence.PreviousPositions[typeof(DefenderMarkerNormalRole4)] = pos;
        ...
        public override void DetermineNextState(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            if (FreekickDefence.CurrentInfos.Any(a => a.RoleType == this.GetType()))
            {
                ...
                if (first)
                {
                    //          if (!FreekickDefence.LastOppToMark4.HasValue || CurrentState == (int)MarkerState.CoverOurGoal)
                    FreekickDefence.LastOppToMark4 = inf.OppID;
                    first = false;
                }
                if (CurrentState == (int)MarkerState.CoverOurGoal)
                {
                    if (OurFreeKickMarkerUtils.PassedToOpponent(Model, RobotID, FreekickDefence.LastOppToMark4))
                    {
                        CurrentState = (int)MarkerState.CoverOppRobot;
                        lastopploc = Model.Opponents[FreekickDefence.LastOppToMark4.Value].Location;
                    }
                }
                else if (CurrentState == (int)MarkerState.CoverOppRobot)
                {
                    if (!OurFreeKickMarkerUtils.PassedToOpponent(Model, RobotID, FreekickDefence.LastOppToMark4) || (FreekickDefence.LastOppToMark4.HasValue && lastopploc.DistanceFrom(Model.Opponents[FreekickDefence.LastOppToMark4.Value].Location) > 0.3))
                    {
        ...
        public override double CalculateCost(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            ...
            if (FreekickDefence.CurrentInfos.Any(a => a.RoleType == this.GetType()))
            {
                ...
                inf = FreekickDefence.CurrentInfos.Where(w => w.RoleType == this.GetType()).First();
                double Teta;
                double cost = CalculateTarget(engine, Model, RobotID, inf, inf.DefenderPosition.Value, inf.Teta, out Teta, FreekickDefence.LastOppToMark4).DistanceFrom(Model.OurRobots[RobotID].Location);
                return cost * cost;
            }
            return 100;
        }
