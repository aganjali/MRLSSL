...
namespace MRL.SSL.AIConsole.Roles
{
    class BusRole3 : RoleBase
    {
        ...
        public void perform(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, int? OppToMark, bool ballIsMoeved, Position2D firstBallPos, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            //if (RegionID == 3 && oppAttackers[RegionID].Count == 1)
            //    RegionID = RegionID - 1;
            //if (RegionID == 1 && oppAttackers[RegionID].Count == 0)
            //    RegionID = RegionID - 1;
            //if (RegionID == 1 && oppAttackers[RegionID].Count == 1 && oppAttackers[RegionID+1].Count == 0 && bus4.HasValue && bus4.Value.RegionID == 2)
            //    RegionID = RegionID - 1;
            //if (oppAttackers[RegionID].Count == 0 && RegionID == 2 && oppAttackers[RegionID - 1].Count > 1)
            //    RegionID = RegionID - 1;
            //else if (RegionID == 1 && oppAttackers[RegionID].Count == 1 && bus3.Value.RegionID == 1 && oppAttackers[RegionID - 1].Count > 2)
            //    RegionID = RegionID - 1;
            ...
            DrawingObjects.AddObject(new StringDraw(((BusState)CurrentState).ToString(), Model.OurRobots[RobotID].Location.Extend(0.2, 0)), "32596898598787");
        }
        ...
        private int? selectOppID(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, int? OppToMark, Position2D firstBallPos, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            if (RegionID == regionNum)
            {
                if (bus2 != null && bus2.RegionID != RegionID)
                {
                    maxAngle = 0;
                    ...
                    foreach (var item in oppAttackers[RegionID])
                    {
                        ...
                        if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) < minAngle)
                        {
                            minAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                            OppID = item.key;
                        }
                    }

                }
                else if (bus4 != null && bus4.RegionID != RegionID)
                {
                    maxAngle = 0;
                    minAngle = double.MaxValue;
                    foreach (var item in oppAttackers[RegionID])
                ...
                else
                {
                    ...
                    {
                        Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                        if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) < minAngle && bus4 != null && bus4.OppID.HasValue && bus4.OppID.Value != item.key && bus2 != null && bus2.OppID.HasValue && bus2.OppID.Value != item.key)
                        {
                            minAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                            OppID = item.key;
                        }
            ...
            else if (RegionID > regionNum)
            {
                ...
                //{
                //    Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                //    if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) > maxAngle && bus2.HasValue && bus2.Value.OppID != item.key && bus1.HasValue && bus1.Value.OppID != item.key)
                //    {
                //        maxAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                //        OppID = item.key;
                //    }
                ...
                {
                    Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                    if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) < minAngle && bus4 != null && bus4.OppID.HasValue && bus4.OppID.Value != item.key )
                    {
                        minAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                        OppID = item.key;
                    }
            ...
            else if (RegionID < regionNum)
            {
                if (bus2 != null && bus2.RegionID == RegionID)
                {
                    ...
                    {
                        Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                        if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) > maxAngle && bus2 != null && bus2.OppID.HasValue && bus2.OppID.Value != item.key)
                        {
                            maxAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                            OppID = item.key;
                        }
                ...
                else
                {
                    ...
                    {
                        Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                        if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) > maxAngle && bus2 != null && bus2.OppID.HasValue && bus2.OppID.Value != item.key && bus1 != null && bus1.OppID.HasValue && bus1.OppID.Value != item.key)
                        {
                            maxAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                            OppID = item.key;
                        }
        ...
        private Position2D? OverlapPosition(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            //if (bus2.HasValue && bus2.Value.Location.DistanceFrom(target) < 0.22 && bus4.HasValue && bus4.Value.Location.DistanceFrom(target) < 0.22)
            //{
            //    if (bus1.HasValue && bus1.Value.Location.DistanceFrom(bus2.Value.Location) < 0.22)
            //    {
            //        double dist = 0.1 + (target.DistanceFrom(Model.OurRobots[bus4.Value.RobotID].Location) / 2);
            //        Vector2D v = (target - bus3.Value.Location).GetNormalizeToCopy(dist);
            //        overlaptarget = Model.OurRobots[bus3.Value.RobotID].Location + v;
            //    }
            //    else if (bus4.Value.Location.X > 4.37 || (bus4.Value.OverlapLocation.HasValue && bus4.Value.OverlapLocation.Value.X > 4.37))
            //    {
            //        double dist = 0.1 + (target.DistanceFrom(Model.OurRobots[bus4.Value.RobotID].Location) / 2);
            //        Vector2D v = (target - bus3.Value.Location).GetNormalizeToCopy(dist);
            //        overlaptarget = Model.OurRobots[bus3.Value.RobotID].Location + v;
            ...
            if (bus2 != null && bus2.target.DistanceFrom(target) < MinDistRobots && bus4 != null && bus4.target.DistanceFrom(target) > MinDistRobots)
            {
                if (target.DistanceFrom(bus2.target) > 0.07)
                    lastBestOverlaptarget = target;
                else
                    target = lastBestOverlaptarget;
                if (bus2 != null && bus2.overlaptarget.HasValue && bus2.overlaptarget.Value.X < 4.36)
                {
                    double dist = 0.1 + (target.DistanceFrom(bus2.target) / 2);
                    Vector2D v = (target - bus2.target).GetNormalizeToCopy(dist);
                    overlaptarget = bus2.target + v;
                }
                else if (bus2 != null && bus2.overlaptarget.HasValue && bus2.overlaptarget.Value.X > 4.36)
                {
                    double dist = 0.2;
                    Vector2D v = (target - Model.OurRobots[bus2ID.Value].Location).GetNormalizeToCopy(dist);
                    overlaptarget = Model.OurRobots[bus2ID.Value].Location + v;
                }
            }
            else if (bus2 != null && bus2.target.DistanceFrom(target) > MinDistRobots && bus4 != null && bus4.target.DistanceFrom(target) < MinDistRobots)
            {
                if (target.DistanceFrom(bus4.target) > 0.02)
                    lastBestOverlaptarget = target;
                else
                    target = lastBestOverlaptarget;
                double dist = 0.1 + (target.DistanceFrom(bus4.target) / 2);
                Vector2D v = (target - bus4.target).GetNormalizeToCopy(dist);
                overlaptarget = bus4.target + v;
                //if (target.DistanceFrom(bus3.target) < 0.02 && lastBestOverlaptarget != Position2D.Zero)
                //    overlaptarget = lastBestOverlaptarget;
                //else
                //    lastBestOverlaptarget = overlaptarget.Value;
            }

            //else if (bus2.HasValue && bus2.Value.Location.DistanceFrom(target) > 0.2 && bus4.HasValue && bus4.Value.Location.DistanceFrom(target) < 0.22)
            //{
            //    if (!bus4.Value.OverlapLocation.HasValue)
            //    {
            //        double dist = 0.1 + (target.DistanceFrom(bus4.Value.Location) / 2);
            //        Vector2D v = (target - bus4.Value.Location).GetNormalizeToCopy(dist);
            //        overlaptarget = bus4.Value.Location + v;
            //    }
            //    else
            //    {
            //        if (Model.OurRobots[bus4.Value.RobotID].Location.DistanceFrom(bus4.Value.Location) < 0.3)
            //        {
            //            double dist = 0.2;// +(target.DistanceFrom(Model.OurRobots[RobotID].Location) / 2);
            //            Vector2D v = (target - Model.OurRobots[bus4.Value.RobotID].Location).GetNormalizeToCopy(dist);
            //            overlaptarget = Model.OurRobots[bus4.Value.RobotID].Location + v;
            //        }
            //        else
            //            overlaptarget = null;
            //    }
            ...
            ///*else*/
            //if (bus2.HasValue && bus2.Value.Location.DistanceFrom(target) < MinDistRobots && bus4.HasValue && bus4.Value.Location.DistanceFrom(target) > 0.2)
            //{
            //    if (bus2.HasValue && bus2.Value.OverlapLocation.HasValue && bus1.HasValue && bus2.Value.OverlapLocation.Value.DistanceFrom(Model.OurRobots[bus1.Value.RobotID].Location) < MinDistRobots)
            //    {
            //        double dist = MinDistRobots;
            //        Vector2D v = (Model.OurRobots[RobotID].Location - Model.OurRobots[bus2.Value.RobotID].Location).GetNormalizeToCopy(dist);
            //        overlaptarget = Model.OurRobots[bus2.Value.RobotID].Location + v;
            //    }
            //    else
            //    {
            //        double dist = 0.1 + (target.DistanceFrom(bus2.Value.Location) / 2);
            //        Vector2D v = ((target.Y < bus2.Value.Location.Y ? target - bus2.Value.Location : bus2.Value.Location - target)).GetNormalizeToCopy(dist);
            //        overlaptarget = bus2.Value.Location + v;
            //    }
            //}
        ...
        public override double CalculateCost(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            ...
            return (Model.OurRobots[RobotID].Location.DistanceFrom(new Position2D(3.22, -0.5)));
        }

        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            List<RoleBase> res = new List<RoleBase>() { new BusRole3(), new NewActiveRole() };
            return res;
        }
