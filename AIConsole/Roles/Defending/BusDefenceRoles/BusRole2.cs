...
namespace MRL.SSL.AIConsole.Roles
{
    class BusRole2 : RoleBase
    {
        ...
        public Position2D lastBestTarget = Position2D.Zero;
        ...
        public void perform(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, int? OppToMark, bool ballIsMoeved, Position2D firstBallPos, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            //if (oppAttackers[RegionID].Count == 0 && RegionID == 2 && oppAttackers[RegionID].Count > 2)
            //    RegionID = RegionID;
            //else if (RegionID == 2 && oppAttackers[RegionID].Count == 1 && bus1.Value.RegionID == 2 && oppAttackers[RegionID].Count > 2)
            //    RegionID = RegionID;
            ...
            DrawingObjects.AddObject(new StringDraw(((BusState)CurrentState).ToString(), Model.OurRobots[RobotID].Location.Extend(0.2, 0)), "kdfryweuyoiq5879");
        }
        ...
        public override void DetermineNextState(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            else if (CurrentState == (int)BusState.Regional)
            {
                ...
                else if (BusBase.BallInBehind(engine, Model, RobotID) && !DataBridge.BallBehind)
                {
                    CurrentState = (int)BusState.BallInBehind;
                }
                else if (GameParameters.IsInDangerousZone(Model.BallState.Location, false, OppFreeKickDefenceUtils.CornerInPenaltyAreaMargin, out d1, out d2))
        ...
        private int? selectOppID(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, int? OppToMark, Position2D firstBallPos, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            {
                if (bus1 != null && bus1.RegionID == RegionID)
                {
                    maxAngle = 0;
                ...
                //else if (bus3.HasValue && bus3.Value.RegionID != RegionID)
                //{
                else
                {
                    maxAngle = 0;
                ...
                //    {
                //        Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                //        if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) > maxAngle && bus1.HasValue && bus1.Value.OppID != item.key)
                //        {
                //            maxAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                //            OppID = item.key;
                //        }
            ...
            {
                //if (bus1.HasValue && bus1.Value.RegionID != RegionID)
                //{
                //    maxAngle = 0;
                //    minAngle = double.MaxValue;
                //    foreach (var item in oppAttackers[RegionID])
                ...
                //    }
                //}
                //else if (bus1.HasValue && bus1.Value.RegionID == RegionID)
                //{
                //    maxAngle = 0;
                //    minAngle = double.MaxValue;
                //    foreach (var item in oppAttackers[RegionID])
                ...
                //else
                {
                    ...
                    {
                        Vector2D oppVec = (Model.Opponents[item.key].Location - GameParameters.OurGoalCenter).GetNormalizeToCopy(5);
                        if (Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID])) < minAngle && bus3 != null && bus3.OppID.HasValue && bus3.OppID.Value != item.key)
                        {
                            minAngle = Math.Abs(Vector2D.AngleBetweenInDegrees(oppVec, RegionVec[RegionID]));
                            OppID = item.key;
                        }
            ...
            {
                if (bus3 != null && bus3.RegionID == RegionID)
                {
                    if (bus1 != null && bus1.RegionID != RegionID)
                    {
                        maxAngle = 0;
        ...
        private Position2D? OverlapPosition(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            //if (bus1.HasValue && bus1.Value.Location.DistanceFrom(target) < MinDistRobots && bus3.HasValue && bus3.Value.Location.DistanceFrom(target) < MinDistRobots)
            //{
            //    if (bus4.HasValue && bus4.Value.Location.DistanceFrom(bus3.Value.Location) < MinDistRobots)
            //    {
            //        double dist = 0.2;// +(target.DistanceFrom(Model.OurRobots[bus2.Value.RobotID].Location) / 2);
            ...
            // 
            /*else*/
            if (bus1 != null && bus1.target.DistanceFrom(target) < MinDistRobots && bus3 != null && bus3.target.DistanceFrom(target) > MinDistRobots)
            {
                if (target.DistanceFrom(bus1.target) > 0.07)
                    lastBestOverlaptarget = target;
                else
                    target = lastBestOverlaptarget;
                if (target.DistanceFrom(Model.OurRobots[bus1ID.Value].Location)<MinDistRobots)
                {
                    target = Model.OurRobots[bus1ID.Value].Location + (target - Model.OurRobots[bus1ID.Value].Location).GetNormalizeToCopy(0.2);
                }
                double dist = 0.1 + (target.DistanceFrom(bus1.target) / 2);
                Vector2D v = (target - bus1.target).GetNormalizeToCopy(dist);
                overlaptarget = bus1.target + v;
                //else if (bus1 != null && bus1.overlaptarget.HasValue && bus1.overlaptarget.Value.X > 4.36)
                //{
                //    double dist = 0.2;// + (target.DistanceFrom(bus1.target) / 2);
                //    Vector2D v = (target - bus1.target).GetNormalizeToCopy(dist);
                //    overlaptarget = bus1.target + v;
                //}
            }
            else if (bus1 != null && bus1.target.DistanceFrom(target) > MinDistRobots && bus3 != null && bus3.target.DistanceFrom(target) < MinDistRobots)
            {
                if (target.DistanceFrom(bus3.target) > 0.07)
                    lastBestOverlaptarget = target;
                else
                    target = lastBestOverlaptarget;
            ...
            ///*else*/
            // if (bus1.HasValue && Model.OurRobots.ContainsKey(bus1.Value.RobotID) && Model.OurRobots[bus1.Value.RobotID].Location.DistanceFrom(target) < MinDistRobots && bus3.HasValue && Model.OurRobots.ContainsKey(bus3.Value.RobotID) && Model.OurRobots[bus3.Value.RobotID].Location.DistanceFrom(target) > MinDistRobots)
            // {
            //     if (bus1.HasValue && bus1.Value.OverlapLocation.HasValue && bus1.Value.OverlapLocation.Value.X > 4.36)
            //     {
            //         double dist = 0.2;
            //         Vector2D v = (Model.OurRobots[RobotID].Location - Model.OurRobots[bus1.Value.RobotID].Location).GetNormalizeToCopy(dist);
            //         overlaptarget = Model.OurRobots[bus1.Value.RobotID].Location + v;
            //     }
            // }
            // /*else*/
            // if (bus1.HasValue && bus1.Value.Location.DistanceFrom(target) > MinDistRobots && bus3.HasValue && bus3.Value.Location.DistanceFrom(target) < MinDistRobots)
            ...
            //     Vector2D v = ((target.Y > bus3.Value.Location.Y ? target - bus3.Value.Location : bus3.Value.Location - target)).GetNormalizeToCopy(dist);
            //     overlaptarget = bus3.Value.Location + v;
            // }


            // if (overlaptarget.HasValue && overlaptarget.Value.DistanceFrom(target) > MinDistRobots / 3)
            // {
            //     if (bus1.HasValue && bus1.Value.OverlapLocation.HasValue && bus1.Value.OverlapLocation.Value.X < 4.36)
            //     {
            //         overlaptarget = null;
        ...
        {
            List<RoleBase> res = new List<RoleBase>() { new BusRole2(), new NewActiveRole() };
            return res;
        }
