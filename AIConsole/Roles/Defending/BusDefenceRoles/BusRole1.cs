...
namespace MRL.SSL.AIConsole.Roles
{
    class BusRole1 : RoleBase
    {
        ...
        public void perform(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, int? OppToMark, bool ballIsMoeved, Position2D firstBallPos, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            OppID = null;
            //if (firstBallPos.Y < 0 && RegionID < 3)
            //{
            //if (oppAttackers[RegionID].Count == 0 && oppAttackers[RegionID].Count > 2)
            //    RegionID = RegionID;
            //else if (oppAttackers[RegionID].Count == 0 && RegionID == 1 && oppAttackers[RegionID].Count == 1 && oppAttackers[RegionID + 2].Count > 2)
            //    RegionID = RegionID;
            //else if (oppAttackers[RegionID].Count == 0 && RegionID == 1 && oppAttackers[RegionID].Count == 0 && oppAttackers[RegionID + 2].Count > 3)
            //    RegionID = RegionID + 2;
            //}
            ...
            if (CurrentState == (int)BusState.Mark && OppID.HasValue && Model.Opponents.ContainsKey(OppID.Value))
            {
                ...
                if (target.DistanceFrom(GameParameters.OurGoalCenter) < 2)
                {
                    lastBestTarget = target;
                }
                else if (lastBestTarget.DistanceFrom(GameParameters.OurGoalCenter) < 2)
                {
                    target = lastBestTarget;
                }
            ...
            {
                //if (!bus2.HasValue || (bus2.HasValue && !bus2.Value.OppID.HasValue))
                {
                    Vector2D v1 = RegionVec[RegionID];
                    Vector2D v2 = RegionVec[RegionID + 1];
                    double d = v2.AngleInDegrees - 5;// (v1.AngleInDegrees >= 0 ? (v1.AngleInDegrees + Math.Abs(Vector2D.AngleBetweenInDegrees(v1, v2)) / 2) : (v1.AngleInDegrees + Math.Abs(Vector2D.AngleBetweenInDegrees(v1, v2)) / 2));
                    ...
                    GameParameters.IsInField(Model.BallState.Location, 0.05);
                }
                //else
                //{
                //    if (Model.OurRobots[RobotID].Location.DistanceFrom(Model.OurRobots[bus2.Value.RobotID].Location) > 0.3)
                //    {
                //        target = Model.OurRobots[bus2.Value.RobotID].Location;
                //        angleDegree = bus2.Value.angleDegree;
                //    }
                //    else
                //    {
                //        target = Model.OurRobots[bus2.Value.RobotID].Location + (Model.OurRobots[RobotID].Location - Model.OurRobots[bus2.Value.RobotID].Location).GetNormalizeToCopy(0.22);
                //        angleDegree = bus2.Value.angleDegree;
                //    }
                //}
            }
            ...
            overlaptarget = OverlapPosition(engine, Model, RobotID, RegionID, oppAttackers, RegionVec, target, AssignedRoles);
            ...
            DrawingObjects.AddObject(new StringDraw(((BusState)CurrentState).ToString(), Model.OurRobots[RobotID].Location.Extend(0.2, 0)), "nSBfvoiaghkjhfougqtgf");
        }
        ...
        private Position2D? OverlapPosition(GameStrategyEngine engine, GameDefinitions.WorldModel Model, int RobotID, int RegionID, Dictionary<int, List<BusRegion>> oppAttackers, List<Vector2D> RegionVec, Position2D target, Dictionary<int, RoleBase> AssignedRoles)
        {
            BusRole1 bus1 = null;
            BusRole2 bus2 = null;
            BusRole3 bus3 = null;
            ...
            if (target.X < 4.38)
            {
                //if (bus2 != null && bus2.overlaptarget.HasValue && bus2.OverlapLocation.Value.DistanceFrom(target) < MinDistRobots)
                //{
                //    if (bus2.HasValue && Model.OurRobots.ContainsKey(bus2.Value.RobotID) && Model.OurRobots[bus2.Value.RobotID].Location.DistanceFrom(target) < MinDistRobots)
                //    {
                //        double dist = 0.1 + (target.DistanceFrom(Model.OurRobots[bus2.Value.RobotID].Location) / 2);
                //        Vector2D v = (target - Model.OurRobots[bus2.Value.RobotID].Location).GetNormalizeToCopy(dist);
                //        overlaptarget = Model.OurRobots[bus2.Value.RobotID].Location + v;
                //    }
                //    else
                //    {
                //        double dist = 0.1 + (target.DistanceFrom(bus2.Value.OverlapLocation.Value) / 2);
                //        Vector2D v = (target - bus2.Value.OverlapLocation.Value).GetNormalizeToCopy(dist);
                //        overlaptarget = bus2.Value.OverlapLocation.Value + v;
                //    }
                //}
                //else 
                if (bus2 != null && bus2.target.DistanceFrom(target) < MinDistRobots)
                {
                    if (target.DistanceFrom(bus2.target) > 0.02)
            ...
            if ((overlaptarget.HasValue && overlaptarget.Value.DistanceFrom(target) > MinDistRobots) || (overlaptarget.HasValue && bus2.target.DistanceFrom(target) > MinDistRobots))
            {
                overlaptarget = null;
            }
            if (target.X > 4.38)
        ...
        {
            List<RoleBase> res = new List<RoleBase>() { new BusRole1(), new NewActiveRole() };
            return res;
        }
