...
{
    public class DefenderCornerRole2 : RoleBase, ISecondDefender
    {
        ...
        public void Run(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D TargetPos, double Teta)
        {
            ...
            //------====||====------- DATA BRIDGE
            DataBridge.CornerRole2ID = RobotID;
            if (inf != null)
            {
                DataBridge.CornerRole2OppID = inf.OppID;
                DataBridge.CornerRole2IsBallTarget = (inf.TargetState.Type == ObjectType.Ball) ? true : false;
            }
            DataBridge.CornerRole2target = Target;
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            int? ballOwner = OppFreeKickDefenceUtils.GetOurBallOwner(engine, Model, RobotID, (DefenderStates)CurrentState, excludeIds);
        ...
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            if (!FreekickDefence.switchAllMode)
            {
                ...
                {
                    List<RoleBase> res = new List<RoleBase>() { new ActiveRole(), new DefenderCornerRole2() };
                    return res;
                }
                else
                {
