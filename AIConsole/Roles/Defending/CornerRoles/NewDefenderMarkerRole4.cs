...
{
    class NewDefenderMarkerRole4 : RoleBase, IMarkerDefender
    {
        ...
        public void mark(GameStrategyEngine engine, WorldModel model, int RobotID, int? MarkID)
        {
            ...
            if (FreekickDefence.OppToMark4.HasValue && model.Opponents.ContainsKey(FreekickDefence.OppToMark4.Value))
            {
                markID = FreekickDefence.OppToMark4.Value;
                markspeed = model.Opponents[FreekickDefence.OppToMark4.Value].Speed;
                markPos = model.Opponents[FreekickDefence.OppToMark4.Value].Location;
                ...
                if (FreekickDefence.OppToMark4.HasValue != null)
                {
                    ...
                    else
                    {
                        ...
                        else if (markPos.DistanceFrom(GameParameters.OurGoalCenter) > Model.OurRobots[RobotID].Location.DistanceFrom(GameParameters.OurGoalCenter) || ((GameParameters.OurGoalCenter - markPos).InnerProduct(Model.OurRobots[RobotID].Location - markPos) > 0.0))
                        {
                            ...
                            else if (CurrentState != (int)MarkState.NearFront)
                            {
                                firstTimeNear = false;
                                avoidness = true;
                                CurrentState = (int)MarkState.IntheWay; //todo:removed farfrint
                            }
                        }
                        else
                        {
        ...
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            if (!FreekickDefence.switchAllMode)
            {
                ...
                {
                    List<RoleBase> res = new List<RoleBase>() { new RegionalDefenderRole(), new NewDefenderMarkerRole4() };
                    return res;
                }
                else
                {
                    List<RoleBase> res = new List<RoleBase>() { new DefenderMarkerRole(),
                                                        ...
                                                        new NewDefenderMarkerRole4()

                                                         };
                    if (FreekickDefence.DefenderMarkerRole4ToActive)
                    {
        ...
        {
            if (FreekickDefence.OppToMark4.HasValue)
            {
                markID = FreekickDefence.OppToMark4.Value;
                markspeed = Model.Opponents[FreekickDefence.OppToMark4.Value].Speed;
                markPos = Model.Opponents[FreekickDefence.OppToMark4.Value].Location;
