...
{
    class NewDefenderMarkerRole2 : RoleBase, IMarkerDefender
    {
        ...
        public void Mark(GameStrategyEngine engine, WorldModel model, int RobotID, int? MarkID)
        {
            ...
            if (FreekickDefence.OppToMark2.HasValue && model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
            {
                ...
                DataBridge.MarkerRole2ID = RobotID;
                DataBridge.MarkerRole2OppID = FreekickDefence.OppToMark2.Value;
                DataBridge.MarkerRole2IsBallTarget = false;
                DataBridge.MarkerRole2target = target;
        ...
        public Position2D CostFunction(WorldModel model, int RobotID, out double retangle, out bool CutBall, bool forTarget)
        {
            if (FreekickDefence.OppToMark2.HasValue && model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
            {
                ...
                CutBall = false;
                markID = FreekickDefence.OppToMark2.Value;
                markspeed = model.Opponents[markID].Speed;
                markPos = model.Opponents[markID].Location;
                ...
                else if (CurrentState == (int)MarkState.Stop)
                {
                    ...
                    if (forTarget)
                        DrawingObjects.AddObject(new StringDraw("STOP", model.OurRobots[RobotID].Location.Extend(0.50, 0.00)), "63863863851322");
                    if (firstTime)
                    {
                        firstTime = false;
                ...
                else if (CurrentState == (int)MarkState.IntheWay)
                {
                    firsttimedanger = true;
                    if (forTarget)
                        DrawingObjects.AddObject(new StringDraw("IN THE WAY", model.OurRobots[RobotID].Location.Extend(0.50, 0)), "87657967577");
                    noIntersect = false;
                ...
                else if (CurrentState == (int)MarkState.FarFront)
                {
                    ...
                    if (intersect.HasValue)
                    {
                        ...
                        if (intersect.Value.DistanceFrom(GameParameters.OurGoalCenter) + 0.25 < model.Opponents[markID].Location.DistanceFrom(GameParameters.OurGoalCenter) && intersect.Value.X < GameParameters.OurGoalCenter.X - .1 && GameParameters.IsInDangerousZone(intersect.Value, false, .2, out dist1, out dist2))// Hey man go to perpendicular point for opponent goes to Blindfold to Goal
                        {
                            ...
                            {
                                Line robotSpeed = new Line(model.OurRobots[RobotID].Location, model.OurRobots[RobotID].Location + ballState.Speed);
                ...
                {
                    if (forTarget)
                        DrawingObjects.AddObject(new StringDraw("goback", model.OurRobots[RobotID].Location.Extend(0.50, 0.00)), "73737637");
                    Line ourGoalCenterOppRobot = new Line(model.Opponents[markID].Location, GameParameters.OurGoalCenter);
                    Line perpendicular = ourGoalCenterOppRobot.PerpenducilarLineToPoint(model.OurRobots[RobotID].Location);
                    Position2D? intersect = ourGoalCenterOppRobot.IntersectWithLine(perpendicular);
                    Position2D extendedtarget = new Position2D();
                ...
                else if (CurrentState == (int)MarkState.NearFront) // Sir you can goes to near of Opponent Robot and mark it with pressure
                {
                    dangerzone = true;
                    if (forTarget)
                        DrawingObjects.AddObject(new StringDraw("NEarFront", model.OurRobots[RobotID].Location.Extend(0.50, 0.00)), "54243534748325");
                    firstTimeAngle = true;
                    ...
                    if (model.Opponents[markID].Speed.Size > 1.00)// I think Im ..... Robot I Pursuit it and .... it in this state
                    {
                        ...
                        marknearmin = min.Center + (GameParameters.OurGoalCenter - min.Center).GetNormalizeToCopy(markDistance); //sPP + (GameParameters.OurGoalCenter - sPP).GetNormalizeToCopy(.18); 
                    }
                    else // Slow move and .... Opponent Robot 
                    {
                        staticAngle = true;
                        angle = (ballState.Location - model.OurRobots[RobotID].Location).AngleInDegrees;
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            if (FreekickDefence.OppToMark2.HasValue && Model.Opponents.ContainsKey(FreekickDefence.OppToMark2.Value))
            {
                ...
                if (intersect.HasValue)
                {
                    ...
                    {
                        counter++;
                        intersectG = GameParameters.OurGoalCenter + (intersectG - GameParameters.OurGoalCenter).GetNormalizeToCopy((intersectG - GameParameters.OurGoalCenter).Size + 0.050);
                        Position2D? ourNearRobot = Model.OurRobots.Where(u => u.Value.Location.DistanceFrom(intersectG) < 0.20).FirstOrDefault().Value.Location;
                        Position2D? oppNearRobot = Model.Opponents.Where(t => t.Value.Location.DistanceFrom(intersectG) < 0.20).FirstOrDefault().Value.Location;
                ...
                if (markPos != Position2D.Zero)
                {
                    ...
                    else
                    {
                        ...
                        else if ((markspeed.GetNormnalizedCopy().InnerProduct((new Vector2D(1, 0)).GetNormnalizedCopy()) > 0.60 && markspeed.Size > 1) &&
                            new Line(markPos + markspeed, markPos).Distance(Model.OurRobots[RobotID].Location) < 0.50 &&
                            (((new Line(markPos + markspeed, markPos).PerpenducilarLineToPoint(Model.OurRobots[RobotID].Location)).IntersectWithLine(new Line(markPos + markspeed, markPos)).Value) - markPos).InnerProduct((markPos + markspeed) - markPos) > 0.00
                        && !cancelInTheWay)
                        {
                        ...
                        DrawingObjects.AddObject(new Line(FarTarget, GameParameters.OurGoalCenter), "654564654646");
                    }
                }
            }
        }
        ...
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            if (!FreekickDefence.switchAllMode)
            {
                if (FreekickDefence.BallIsMoved && FreekickDefence.WeAreInCorner)
                {
                    ...
                    if (FreekickDefence.SwitchDefender32Marker2)
                    {
                    ...
                    if (FreekickDefence.LastSwitchDefender32Marker2)//New IO2014
                    {
                        res.Add(new DefenderCornerRole3());
                    }
                    return res;
