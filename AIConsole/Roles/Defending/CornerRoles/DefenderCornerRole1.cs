...
{
    public class DefenderCornerRole1 : RoleBase, IFirstDefender
    {
        ...
        public void Run(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D TargetPos, double Teta)
        {
            ...
            Target = Cost(engine, Model, RobotID, TargetPos, Teta, out teta, inf); //Calculate New Positions
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            //var excludeIds = AssignedRoles.Where(w => w.Value.GetType() == typeof(DefenderCornerRole2)).Select(s => s.Key).ToList(); // Iran Open 2015
            bool NormalFromBehind = false; //  exit from Behind to Normal with histersis

            //update ballowner
            int? ballOwner = OppFreeKickDefenceUtils.GetOurBallOwner(engine, Model, RobotID, (DefenderStates)CurrentState, null);

            if (CurrentState != (int)DefenderStates.Behind)
            {
                OppFreeKickDefenceUtils.ResetBehindstate();
            }
            ...
            if (inf != null && inf.DefenderPosition.HasValue && weHaveInitialState)
            {
                ...
                {
                    if (ballcoeff > 0 && ballcoeff > 0// time horizons
                ...
                if (gotointermediatepos) // update important parameters
                {
                    if (intermediatePos != new Position2D())
                    {
                        DataBridge.BallCutSituationCR1 = true;
                        DataBridge.CutBallRobotIDCR1 = RobotID;
                        TargetDefenderCutr = inf.DefenderPosition.Value;
                        InitialDefenderCutr = initialpos;
            ...
            else if (CurrentState == (int)DefenderStates.Normal)
            {
                ...
                else if (DataBridge.BallBehind && DataBridge.BallBehindID == RobotID)
                {
                    CurrentState = (int)DefenderStates.Behind;
                }

                else if (gotoBreakpos)
                {
                    CurrentState = (int)DefenderStates.Break;
                }
            ...
            else if (CurrentState == (int)DefenderStates.KickToGoal)
            {
                if (!OppFreeKickDefenceUtils.BallKickedToGoal(engine, Model))
                {
                    if (GameParameters.IsInDangerousZone(ballState.Location, false, OppFreeKickDefenceUtils.CornerInPenaltyAreaMargin, out dist, out dist2))
                    {
                        CurrentState = (int)DefenderStates.InPenaltyArea;
                    }
...
if (!calculateCost)
        ...
        public Position2D Cost(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D TargetPos, double Teta, out double teta, DefenceInfo inf)
        {
            ...
            else if (CurrentState == (int)DefenderStates.Normal)
            {
                ...
                if (inf != null && inf.TargetState.Speed.Size > 1)
            ...
            if (inf != null)
            {
                if ((CurrentState == (int)DefenderStates.Normal && inf.TargetState.Location.X > GameParameters.OurGoalCenter.X) || (CurrentState == (int)DefenderStates.Behind && inf.TargetState.Location.X > GameParameters.OurGoalCenter.X))
                {
