...
{
    class DefenderCornerRole3 : RoleBase, ISecondDefender
    {
        ...
        public void Run(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D TargetPos, double Teta)
        {
            ...
            FreekickDefence.PreviousPositions[typeof(DefenderCornerRole3)] = Target;
            //------====||====------- DATA BRIDGE
            DataBridge.CornerRole1ID = RobotID;
            if (inf != null)
            {
                DataBridge.CornerRole1OppID = inf.OppID;
                DataBridge.CornerRole1IsBallTarget = (inf.TargetState.Type == ObjectType.Ball) ? true : false;
            }
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            int? ballOwner = OppFreeKickDefenceUtils.GetOurBallOwner(engine, Model, RobotID, (DefenderStates)CurrentState, null);
            ...
            if (inf != null && inf.DefenderPosition.HasValue && weHaveInitialState)
            {
                ...
                if (gotointermediatepos) // update important parameters
                {
                    if (intermediatePos != new Position2D())
                    {
                        DataBridge.BallCutSituationCR3 = true;
                        DataBridge.CutBallRobotIDCR3 = RobotID;
                        TargetDefenderCutr = inf.DefenderPosition.Value;
                        InitialDefenderCutr = initialpos;
            ...
            {
                if (DataBridge.CutBallRobotIDCR3 != RobotID)
                {
                    CurrentState = (int)DefenderStates.Normal;
                }
            ...
            else if (CurrentState == (int)DefenderStates.Normal)
            {
                ...
                else if (DataBridge.CutBallRobotIDCR3 == RobotID && DataBridge.BallCutSituationCR3 && DataBridge.staticCutAssign)
                {
                    CurrentState = (int)DefenderStates.Cut;
                }
            ...
            {
                if (DataBridge.CutBallRobotIDCR3 == RobotID && DataBridge.BallCutSituationCR3 && DataBridge.staticCutAssign)
                {
                    CurrentState = (int)DefenderStates.Cut;
                }
                else if (BallCutSituationr && !DataBridge.staticCutAssign)
            ...
            else if (CurrentState == (int)DefenderStates.BallInFront)
            {
                ...
                if (DataBridge.CutBallRobotIDCR3 == RobotID && DataBridge.BallCutSituationCR3)
                {
                    CurrentState = (int)DefenderStates.Cut;
                }
            ...
            //{
            //    if (DataBridge.CutBallRobotID == RobotID && DataBridge.BallCutSituation)
            //    {
            //        CurrentState = (int)DefenderStates.Cut;
            //    }
            ...
            if (CurrentState != (int)DefenderStates.BallInFront)
                counterBalInFront = 0;
            DrawingObjects.AddObject(new StringDraw(((DefenderStates)CurrentState).ToString(), Model.OurRobots[RobotID].Location + new Vector2D(0.25, 0)), "Def325146545649865");
            if (!calculateCost)
        ...
        int counter2 = 0;
        SingleObjectState BehindTarget(WorldModel Model, int RobotID)
        {
            if (firstTime4)
            {
                firstTime4 = false;
        ...
        public Position2D Cost(GameStrategyEngine engine, WorldModel Model, int RobotID, Position2D TargetPos, double Teta, DefenceInfo inf, out double teta)
        {
            ...
            {
                DataBridge.BallCutSituationCR2 = true;
            ...
            if (inf != null)
                if ((CurrentState == (int)DefenderStates.Normal && inf.TargetState.Location.X > GameParameters.OurGoalCenter.X))
                {
        ...
        public override List<RoleBase> SwichToRole(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> previouslyAssignedRoles)
        {
            if (FreekickDefence.switchAllMode)
            {
                ...
                {
                    List<RoleBase> res = new List<RoleBase>() { new ActiveRole(), new DefenderCornerRole3() };
                    return res;
                }
                else
                {
