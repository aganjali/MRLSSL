...
namespace MRL.SSL.AIConsole.Roles
{
    public class NewActiveRole : RoleBase
    {
        ...
        public void Perform(GameStrategyEngine engine, WorldModel Model, int RobotID, bool kickAnyway)
        {
            ...
            if ((Model.OurRobots[RobotID].Location.DistanceFrom(Model.BallState.Location) < 0.5 || GetSkill<GetBallSkill>().CurrState == NormalSharedState.GetBallState.Incomming) && currentActionDesc.ActionCategory() != ActiveActionMode.Pass)
            {
                ActiveInfo.spin = true;
            }
            else
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            Type type;
            ActionDescriptionBase selectedAction = FindBestAction(engine, Model, RobotID, out type, ref actInfo);
        ...
        public void SpinRotate(GameStrategyEngine engine, WorldModel Model, int RobotID)
        {
            ...
            {
                GetSkill<ActiveRotateSkill>().Rotate(Model, RobotID, ActiveInfo.KickTarget, ActiveInfo.kickSpeed, Model.BallState.Location.Y > 0);
            }
        }
