    ...
    public class AttackerRole2017 : RoleBase
    {
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            if (attackerPos.HasValue)
            {
                ...
                List<Planning.GamePlanner.Types.PassPointData> poses = engine.GameInfo.CalculateAttackerPassScore(Model, NormalSharedState.CommonInfo.ActiveID.Value, RobotID, attackerPos.Value, topLeft, passSpeed, shootSpeed, width, heigth, Rows, column);
            ...
            //    double width = GameParameters.OurGoalCenter.X - 0.5 - 0.25, heigth = 2 * GameParameters.OurLeftCorner.Y, passSpeed = 4, shootSpeed = 8;
            //    int Rows = 5, column = 10;
            //    List<Planning.GamePlanner.Types.PassPointData> poses = engine.GameInfo.CalculatePassScore(Model, NormalSharedState.CommonInfo.ActiveID.Value, RobotID, topLeft, passSpeed, shootSpeed, width, heigth, Rows, column);
            //    double maxSc = double.MinValue;
            //    foreach (var item in poses)
            //    {
            //        if (item.score > maxSc)
