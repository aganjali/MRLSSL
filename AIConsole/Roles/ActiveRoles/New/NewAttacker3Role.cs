...
{
    public class NewAttacker3Role : RoleBase
    {
        bool Debug = true;
        ...
        public override void DetermineNextState(GameStrategyEngine engine, WorldModel Model, int RobotID, Dictionary<int, RoleBase> AssignedRoles)
        {
            ...
            bool mode = NormalSharedState.CommonInfo.AttackerMode;
            ...
            {
                if (Model.OurRobots[RobotID].Location.DistanceFrom(markPoint) < goTowardBallMarg)
                    CurrentState = (int)AttackerState.MarkGoTowardBall;
            }
            else if (CurrentState == (int)AttackerState.MarkGoTowardBall)
            {

                if (!oppMarkId.HasValue || NormalSharedState.AttackerInfo.OppMarkID.HasValue && oppMarkId.Value != NormalSharedState.AttackerInfo.OppMarkID.Value)
                {
                    if (Model.BallState.Location.Y > 0)
                    {
                        markPoint = new Position2D(-3.5, -2);
                    }
                    else 
                    {
                        markPoint = new Position2D(-3.5, 2);
            ...
            if (CurrentState == (int)AttackerState.WaitForPass)
            {
                ...
                firstInWait = false;
                //bool reset = false;
                //if (!lastBallPos.HasValue || (lastBallPos.HasValue && Model.BallState.Location.DistanceFrom(lastBallPos.Value) > 0.6))
                //    reset = true;

                //var poses = NormalSharedState.CommonInfo.GetPassPoints(reset, reset, engine, Model, Model.BallState.Location, GameParameters.OppGoalCenter, new Position2D(0.75, GameParameters.OurRightCorner.Y),
                //    new Vector2D(2.5, 2 * GameParameters.OurLeftCorner.Y), 5, 10, 5, Program.MaxKickSpeed);

                //if (!lastBallPos.HasValue)
                //    lastBallPos = Model.BallState.Location;
                //if (poses.Count > 1)
                //    markPoint = Position2D.Interpolate(poses[0], poses[1], 0.5);
                //else if (poses.Count > 0)
                //    markPoint = poses[0];
                //else
                //    markPoint = Position2D.Zero;
                ...
                //    {
                //        DrawingObjects.AddObject(new Circle(item, 0.1, new Pen((count == 0) ? Color.Red : Color.Blue, 0.01f)), "p" + item + count++);
                //    }
                //}
            }
            else if (CurrentState == (int)AttackerState.MarkGotoPoint)
            {
                ...
                {
                    if (Model.BallState.Location.Y >1.5)
                    {
                        markPoint = new Position2D(-3.5, -2);
                    }
                    else if (Model.BallState.Location.Y < -1.5)
                    {
                        markPoint = new Position2D(-3.5, 2);
                    }
                }
            ...
            else if (CurrentState == (int)AttackerState.MarkGoTowardBall)
            {
                ...
                {
                    Vector2D ballOppVec = (Model.BallState.Location - Model.Opponents[oppMarkId.Value].Location);
                    Position2D p = Model.Opponents[oppMarkId.Value].Location + ballOppVec * 0.5;
        ...
        {
            return new List<RoleBase>() { new NewAttacker2Role(), new NewAttacker3Role(), new NewActiveRole(), new NewSupporter2Role() };
        }
