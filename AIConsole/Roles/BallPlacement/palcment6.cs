    ...
    class palcment6 : RoleBase
    {
        ...
        public SingleWirelessCommand RunRole(GameStrategyEngine engine, MRL.SSL.GameDefinitions.WorldModel Model, int RobotID)
        {
            ...
            Planner.SetParameter(RobotID, 1, 0.7);
                return GetSkill<GotoPointSkill>().GotoPoint(Model, RobotID, GetTarget(Model, RobotID, ballState), (ballState.Location - Model.OurRobots[RobotID].Location).AngleInDegrees,true,true,2, false, true,false, new List<Obstacle>() {
                    new Obstacle()
                    {
        ...
        private Position2D GetTarget(WorldModel Model, int RobotID, SingleObjectState ballfakepos)
        {
            ...
            tar = target;
            DrawingObjects.AddObject(new Circle(target, 0.04, new Pen(Color.Yellow, 0.01f)), "ballcirc6");
            ...
            //#endregion
            #endregion
            //DrawingObjects.AddObject(new StringDraw("target6= " + target, new Position2D(3.5, 5)), "target6");
            Position2D p1, p2;
            double dist1, dist2;

            ll = new Line(StaticVariables.ballPlacementPos, Model.BallState.Location);
            // DrawingObjects.AddObject(new Line(StaticVariables.ballPlacementPos, Model.BallState.Location, new Pen(Color.BlueViolet, 0.01f)), "l16");

            Position2D p = (ll).PerpenducilarLineToPoint(target).IntersectWithLine(ll).Value;
            //DrawingObjects.AddObject(new Circle(p, 0.04, new Pen(Color.HotPink, 0.01f)), "p6");

            Line l2 = new Line(target, p);
            //DrawingObjects.AddObject(new Line(target, p, new Pen(Color.BlueViolet, 0.01f)), "l26");
            ...
            if (Model.BallState.Location.X > StaticVariables.ballPlacementPos.X)
            {
                ...
                {
                    Target.X = p.X;
                    //DrawingObjects.AddObject(new StringDraw("Target.Xb6= " + Target.X, new Position2D(4, 5)), "Target.Xb6");
                    if (Model.BallState.Location.Y > StaticVariables.ballPlacementPos.Y)
                    {
                        ...
                        {
                            Target.Y = p.Y;
                            //DrawingObjects.AddObject(new StringDraw("Target.Yb6= " + Target.Y, new Position2D(4.5, 5)), "Target.Yb6");
                        }
                    }
                    else if (Model.BallState.Location.Y < StaticVariables.ballPlacementPos.Y)
                    {
                        ...
                        {
                            Target.Y = p.Y;
                            //DrawingObjects.AddObject(new StringDraw("Target.Yp6= " + Target.Y, new Position2D(4.5, 5)), "Target.Yp6");
                        }
                    }
                }
                if (target.DistanceFrom(Target) < 0.65)
                {
                    ...
                    // DrawingObjects.AddObject(new Circle(Target, 0.65, new Pen(Color.Khaki, 0.01f)), "cTarget6");
                    line1 = new Line(target, Target);
                    //DrawingObjects.AddObject(new Line(target, Target, new Pen(Color.BlueViolet, 0.01f)), "target6");
                    List<Position2D> pos = cTarget.Intersect(line1);
                    p1 = pos.First();
                    p2 = pos.Last();
                    //DrawingObjects.AddObject(new Circle(p1, 0.04, new Pen(Color.White, 0.01f)), "cTargetr6");
                    //DrawingObjects.AddObject(new Circle(p2, 0.04, new Pen(Color.Red, 0.01f)), "cTargett6");
            ...
            else if (Model.BallState.Location.X < StaticVariables.ballPlacementPos.X)
            {
                ...
                {
                    Target.X = p.X;
                    // DrawingObjects.AddObject(new StringDraw("Target.Xp6= " + Target.X, new Position2D(4, 5)), "Target.Xp6");
                    if (Model.BallState.Location.Y > StaticVariables.ballPlacementPos.Y)
                    {
                        if (p.Y < Model.BallState.Location.Y && p.Y > StaticVariables.ballPlacementPos.Y)
                        {
                ...
                {
                    Circle cTarget = new Circle(Target, .65);
                    // DrawingObjects.AddObject(new Circle(Target, 0.04, new Pen(Color.Khaki, 0.01f)), "cTarget6");
                    line1 = new Line(target, Target);
                    List<Position2D> pos = cTarget.Intersect(line1);
                    p1 = pos.First();
                    p2 = pos.Last();
            ...
            else
            {
                ...
                else
                {
                    ...
                    // DrawingObjects.AddObject(new Circle(Model.BallState.Location, 0.04, new Pen(Color.White, 0.01f)), "cTarget6");
                ...
                else
                {
                    ...
                    // DrawingObjects.AddObject(new Circle(StaticVariables.ballPlacementPos, 0.04, new Pen(Color.White, 0.01f)), "cTarget6");
            ...
            DrawingObjects.AddObject(new StringDraw("target6= " + tar, new Position2D(4, 5)), "target6");
            targetOverLap6 = tar;
            return tar;
        }
        ...
        {
            return new List<RoleBase>() { new palcment6(), new ActiveRole() };
        }
