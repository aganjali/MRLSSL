...
struct GVector2D
{
	__device__ GVector2D()
	{
	}
	float X;
	float Y;
	__device__ GVector2D(float x, float y)
	{
		X = x;
		Y = y;
	}
	__device__ GVector2D(GVector2D *vec)
	{
		X = vec[0].X;
		Y = vec[0].Y;
	}
	__device__ float Size()
	{
		return sqrtf(X * X + Y * Y);
	}
	__device__ float SquareSize()
	{
		return X * X + Y * Y;
	}
	__device__ bool Scale(float a)
	{
		X *= a;
		Y *= a;
		return true;
	}
	__device__ bool Normnalize()
	{
		float num = Size();
		if (num == 0)
		{
	...
	__device__ bool NormalizeTo(float NewLength)
	{
		float num = Size();
		if (num == 0)
		{
	...
	__device__ GVector2D GetNormnalizedCopy()
	{
		GVector2D result = GVector2D(X, Y);
		result.Normnalize();
		return result;
	}
	__device__ GVector2D GetNormalizeToCopy(float NewLength)
	{
		GVector2D result = GVector2D(X, Y);
		result.NormalizeTo(NewLength);
		return result;
	}
	__device__ float AngleBetweenInRadians(GVector2D P1)
	{
	...
	__device__ float AngleBetweenInDegrees(GVector2D P1)
	{
		return AngleBetweenInRadians(P1) * 180 / 3.141593;
	}
	__device__ float InnerProduct(GVector2D V)
	{
		return X * V.X + Y * V.Y;
	}
	__device__ GVector2D FromAngleSize(float Angle, float Size)
	{
		return GVector2D(Size * cosf(Angle), Size * sinf(Angle));
	}
	__device__ float AngleInRadians()
	{
		return atan2f(Y, X);
	}
	__device__ float AngleInDegrees()
	{
		return AngleInRadians() * 180 / 3.141593;
	}
	__device__ GVector2D Add(GVector2D v1)
	{
		return GVector2D(X + v1.X, Y + v1.Y);
	}
	__device__ GVector2D Sub(GVector2D v1)
	{
		return GVector2D(X - v1.X, Y - v1.Y);
	}
...
struct GPosition2D
{
	__device__ GPosition2D()
	{
	}
	float X;
	float Y;
	__device__ GPosition2D(float x, float y)
	{
		X = x;
		Y = y;
	}
	__device__ GPosition2D(GPosition2D *pos)
	{
		X = pos[0].X;
		Y = pos[0].Y;
	}
	...
	__device__ float DistanceFrom(GPosition2D From)
	{
		return sqrtf(((X - From.X) * (X - From.X) + (Y - From.Y) * (Y - From.Y)));
	}
	__device__ GPosition2D Add(GVector2D p1)
	{
	...
	__device__ GPosition2D Interpolate(GPosition2D End, float Amount)
	{
...
struct GObjectState
{
	...
	__device__ GObjectState(GObjectState *From)
	{
		Type = From[0].Type;
	...
	__device__ GObjectState(GObjectType type, GPosition2D location, GVector2D speed, GVector2D acceleration)
	{
		Type = type;
		Location = location;
		Speed = speed;
	...
	__device__ GObjectState(GPosition2D location, GVector2D speed)
	{
		Location = location;
		Speed = speed;
